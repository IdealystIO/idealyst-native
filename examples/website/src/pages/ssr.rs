...
fn render_on_server() -> Element {
    ...
    ui! {
        Section(
            title = "Render on the server".to_string(),
            paragraphs = vec![
                "The SSR backend is just another `Backend`. It walks the same primitive \
                 ...
                 needs \u{2014} for any route, on the host, with no browser involved.".to_string(),
                "Because it renders at a path, a complex app with a navigator renders \
                 ...
                 full isolation with no cross-render contamination.".to_string(),
            ],
