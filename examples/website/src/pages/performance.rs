        ...
        Section(
            ...
            paragraphs = vec![
                "Most UI frameworks pay an abstraction tax at render time: a virtual DOM is \
                 ...
                 every update, and competes with your app for the main thread.".to_string(),
                ...
                "Two more multipliers compound it. Ahead-of-time compilation to native code \
                 ...
                 alongside your app and nothing to initialize before the first frame.".to_string(),
            ],
        )
    }
}
...
fn fine_grained() -> Element {
    ...
    ui! {
        Section(
            title = "Fine-grained, not coarse".to_string(),
            paragraphs = vec![
...
fn how_measured() -> Element {
    ui! {
        Section(
            title = "How we measure".to_string(),
            paragraphs = vec![
                "Performance claims are easy to make and easy to fudge, so the benchmark \
                 ...
                 The rules that keep it honest:".to_string(),
                "Production builds everywhere. React runs the production esm.sh bundle \
                 ...
                 Dev-mode equivalents are 2\u{2013}5\u{00d7} slower and are never reported.".to_string(),
                "A strict resolution contract. The measured `setRows(n)` call must resolve \
                 ...
                 being batched away afterward.".to_string(),
                "Honest baselines bracket the result. Three hand-written vanilla variants \
                 ...
                 somewhere on that spectrum, and you can see exactly where.".to_string(),
            ],
        )
    }
}
...
fn compared_against() -> Element {
    ...
    ui! {
        Stack(gap = StackGap::Md) {
            Typography(
                content = "What it's compared against".to_string(),
                kind = idea_ui::typography_kind::H2,
            )
            Typography(
                content = "The suite ships eight variants spanning the realistic \
...
fn reproduce() -> Element {
    ...
    ui! {
        Stack(gap = StackGap::Md) {
            Typography(
                content = "Reproduce it yourself".to_string(),
                kind = idea_ui::typography_kind::H2,
            )
            Typography(
                content = "We publish the harness, not just a headline. Numbers depend \
            ...
            CodePanel(src = snippet.to_string())
            Typography(
                content = "If you want to add your own framework to the comparison, the \
