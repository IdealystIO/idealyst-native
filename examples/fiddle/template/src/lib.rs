    ...
    fn simulator_tree() -> runtime_core::Element {
        let graphics = runtime_core::primitives::graphics::graphics(move |event: OnReadyEvent| {
            wasm_bindgen_futures::spawn_local(async move {
                ...
                let size = event.size;
                // `OnReadyEvent.target` is an enum since the GL work — a GL
                // target has no raw window handle, so step aside rather than
                // fabricate a surface.
                ...
                match host_web::mount_newcore(surface, size, profile, skin, build)
                    .await
                {
