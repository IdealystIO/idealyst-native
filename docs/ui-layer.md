...
`framework_core::Primitive` is an enum — one variant per "kind of thing
...
1. Emit calls to `framework_core::{text, button, view, when, switch}`
...
| Goal | Where it lives |
| --- | --- |
| Add a new built-in primitive | New `Primitive` variant + walker arm in `framework_core::lib`, plus a `create_*` / `update_*` method on `Backend` |
