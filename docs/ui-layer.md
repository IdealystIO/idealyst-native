...
| a literal in a `ui!` nested in a closure prop (`content = move \|\| { ui! { … } }`) | **patched**, addressed to the NESTED site. The text an enclosing site records for its closures, bare-expression children and slots has every nested `ui!` body blanked out: the nested site is diffed on its own, and an edit inside it no longer reads as the enclosing site's code changing (it did, and every literal edit in a dialog body was a hot patch). Code in the closure outside the nested `ui!` still counts |
