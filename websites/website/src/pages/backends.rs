...
fn matrix() -> Element {
    let rows: Vec<(&str, &str, &str)> = vec![
        ...
        ("CPU graphics (Arduino, ESP32)", "Working (MVP)", "Software rasterizer with custom `Surface` trait. Full support for View, Text, Button, Pressable, scroll_view (gradients, rounded corners, hit-testing). Inputs, lists, GPU, modals, navigators render a visible placeholder text per `feedback_cpu_unsupported_placeholders` — missing support is SEEN on device, not silently no-op'd."),
