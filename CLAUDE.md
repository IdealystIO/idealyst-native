...
**Primitives are lowercase, components are PascalCase — strictly.** Inside `ui!`/`jsx!`, the framework's leaf primitives (`view`, `text`, `button`, `image`, `icon`, `text_input`, `scroll_view`, `slider`, `toggle`, `link`, `overlay`, `anchored_overlay`, `presence`, `activity_indicator`, `flat_list`, `when`, `graphics`) are snake_case **only**. The macro no longer accepts the legacy PascalCase spelling of a primitive — a PascalCase tag (`View`, `Image`, `Link`, `Toggle`, …) is **always** routed to `#[component]` (`BuildElement`) dispatch. This is what frees a component library or app to define a `#[component]` named `Image`/`Link`/`Toggle`/etc. without the same-named primitive shadowing it (see `canonical_primitive` in `crates/runtime/macros/src/primitives.rs`). The whole tree (framework crates + tests + backends + SDK + examples) has been migrated to snake_case primitive tags; write new call sites lowercase and convert any stray PascalCase primitive you find. This mirrors React's `<div>` vs `<MyButton>` convention.
