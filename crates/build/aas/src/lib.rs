...
fn generate_sidecar_wrapper(
    ...
    // (`__<Name>_hot_impl` + outer dispatch via `framework_hot::call`).
    ...
    // `aas-runtime` is dev-server's opt-in for both `host::run` and
    // `sidecar::run`. It pulls `framework-hot`, `subsecond-types`,
    // `libc`, and `anyhow` into the wrapper transitively — we no
    // longer name those deps from this Cargo.toml.
    let dev_server_dep = source.dep("crates/dev/server", &["aas-runtime"]);
...
# `dev_server::sidecar::run` to host the AAS frame loop. Pre-refactor
# this crate carried framework-hot, wire, subsecond-types, libc, and
...
//! split-process AAS dev host. Delegates the entire frame loop to
...
# every build.
#
# Infra-only AAS host: WebSocket server, mDNS, file watcher, and
...
# `aas-runtime` feature); this wrapper exists only to construct a
# `HostConfig` and wire `build_aas::hotpatch::HotPatchBuilder` to
...
"#,
    ...
    let main_rs = format!(
        r#"//! GENERATED by `idealyst build aas`. Thin AAS host shim:
...
//! `dev-server` under its `aas-runtime` feature — only the
//! project-specific paths are inlined here.

use std::path::PathBuf;
use build_aas::hotpatch::{{HotPatchArtifact, HotPatchBuilder}};
...
fn main() -> std::io::Result<()> {{
    ...
    let hot_patch: Option<Box<dyn HotPatchAdapter>> =
        match HotPatchBuilder::new(captures_dir, &sidecar_path, patch_target_dir) {{
            ...
            Err(e) => {{
                eprintln!(
                    "[aas-host] hot-patch builder init failed: {{e:#}} — \
