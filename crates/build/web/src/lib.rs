...
pub fn generate_wrapper(
    ...
    // `dev-client` is only needed in AAS mode. Declared as an
...
# AAS-mode dep. Optional + gated by the `aas` feature so plain wasm
...
# AAS-mode `start()` calls `js_sys::Reflect::get` to read
# `window.IDEALYST_AAS_URL` and `web_sys::console` for log lines.
...
[features]
# AAS-mode hot reload. Activated by `idealyst dev --aas --web`. When
# on, the generated `start()` reads `window.IDEALYST_AAS_URL` (the dev
# HTTP server injects it on every served page) and connects the
# `WebBackend` to the AAS host over WebSocket via
...
# replay engine) AND turn on `backend-web/aas-shell`, which is what
# gates the `connect_web` + `WebClientHandle` exports we use below.
aas = ["dep:dev-client", "backend-web/aas-shell"]
...
//!
//! - **`aas` feature on:** reads `window.IDEALYST_AAS_URL` (the dev
//!   HTTP server injects it into every served page) and connects a
//!   `WireBackend<WebBackend>` to the AAS host over WebSocket via
//!   `backend_web::connect_web`. The browser becomes a thin replayer;
//!   the framework runtime lives in the AAS sidecar. This is what
//!   `idealyst dev --aas --web` produces. Without the feature the
//!   browser would render locally and never connect to AAS — the
...
thread_local! {{
    ...
    /// AAS-mode: the `WebClientHandle` owns the WebSocket + event
    ...
    static AAS_HANDLE: RefCell<Option<backend_web::WebClientHandle>> =
        const {{ RefCell::new(None) }};
    /// AAS-mode: the `WireBackend` lives behind an `Rc<RefCell<…>>`
...
/// AAS mode: framework runtime lives in the AAS sidecar on the dev
...
fn start_aas() {{
    let url = match read_aas_url() {{
        ...
        None => {{
            web_sys::console::error_1(
                &"[dev-client] AAS mode enabled but window.IDEALYST_AAS_URL is missing — \
    ...
    web_sys::console::log_1(
        &format!("[dev-client] AAS mode: connecting to {{}}", url).into(),
    );
    ...
    match backend_web::connect_web(&url, wire, on_disconnect) {{
        ...
        Err(e) => web_sys::console::error_2(
            &"[dev-client] initial AAS connect failed:".into(),
            &e,
        ),
    }}
}}

/// Read `window.IDEALYST_AAS_URL`, the URL the dev HTTP layer injects
/// into the page via `<script>window.IDEALYST_AAS_URL = "..."</script>`.
#[cfg(feature = "aas")]
fn read_aas_url() -> Option<String> {{
    let win = web_sys::window()?;
    js_sys::Reflect::get(&win, &"IDEALYST_AAS_URL".into())
        .ok()?
        .as_string()
}}
"##,
