...
use port_preview::{discover_framework_core, preview, CheckResult, PreviewConfig};
...
fn main() -> ExitCode {
    ...
    // Resolve framework-core. Explicit `--framework-path` wins;
    ...
    let framework_path = match args.framework_path {
        ...
        None => {
            ...
            match discover_framework_core(&here) {
                Some(p) => p,
                None => {
                    eprintln!(
                        "port-preview: could not find framework-core. \
    ...
    let outcome = match preview(&PreviewConfig {
        ...
        framework_core_path: &framework_path,
    }) {
        Ok(o) => o,
        Err(e) => {
