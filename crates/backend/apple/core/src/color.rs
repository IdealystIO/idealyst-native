...
//! tuple step is pure (lives in `runtime_shared::color`); this
...
/// [`runtime_shared::color::parse_or`]; this wrapper exists so the
/// leaf crate's `color_to_uicolor` / `color_to_nscolor` adapter
/// stays a one-liner.
pub fn parse_color(s: &str) -> (CGFloat, CGFloat, CGFloat, CGFloat) {
    let [r, g, b, a] = runtime_shared::color::parse_or(s, runtime_shared::color::Rgba::BLACK)
        .to_srgb_f32();
