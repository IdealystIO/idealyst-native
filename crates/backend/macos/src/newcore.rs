...
impl Host for MacosBackend {
    ...
    fn create_anchor(&mut self) -> Self::Node {
        // A layout- and hit-transparent view over a `display: contents`
        // layout node — this backend's `display: contents`. A plain view
        // here is a flex item the author never wrote; see
        // `create_anchor_impl`.
        MacosBackend::create_anchor_impl(self)
    }
