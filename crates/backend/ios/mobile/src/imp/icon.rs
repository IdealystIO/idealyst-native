...
pub(crate) fn create_icon(
    ...
    let cg_icon: CGColorRef = unsafe { msg_send![&icon_color, CGColor] };
    ...
    if data.filled {
        ...
        // disable the stroke. `update_icon_color` mirrors this by writing
        // fillColor when fillColor is non-clear.
        let _: () = unsafe { msg_send![&shape_layer, setFillColor: cg_icon] };
        let _: () = unsafe { msg_send![&shape_layer, setStrokeColor: cg_clear] };
    } else {
        // Stroke-only (Lucide / outlined icon style): stroke the paths,
        // leave the interior transparent.
...
/// Update the icon's color. Grabs the first sublayer (our CAShapeLayer)
/// and rewrites whichever paint is active: a filled icon has a non-clear
/// `fillColor` (set at create time) so we update `fillColor`; an outlined
/// icon has a clear fill so we update `strokeColor`.
...
/// meaning the icon was created in `filled` mode. A clear/null fillColor
/// has alpha 0.
fn shape_layer_is_filled(shape: &Retained<NSObject>) -> bool {
    let cg_fill: CGColorRef = unsafe { msg_send![shape, fillColor] };
    if cg_fill.0.is_null() {
        return false;
    }
