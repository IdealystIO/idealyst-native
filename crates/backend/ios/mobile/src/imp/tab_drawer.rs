...
pub(crate) fn create_drawer_navigator(
    ...
    // the subscription's lifetime matches the drawer's.
    {
        ...
        let theme_effect = runtime_core::Effect::new(move || {
            ...
            // closures don't read any tokens at all, this effect
            // settles after its first run (which is the desired
            // outcome — nothing reactive to react to).
    ...
    if let Some(bg_closure) = callbacks.background_color.clone() {
        ...
        let bg_effect = runtime_core::Effect::new(move || {
            ...
            // reads. A hardcoded-color closure produces no
            // subscriptions and the effect settles after its first
            // run.
            let color = (bg_closure)();
            let ui_color = color_to_uicolor(&color);
            // Snap the nav-bar + body background. With the active-
            // theme concept removed, there's no global "theme swap
            // in progress" flag to opt into a transition here — if
            // an author wants the swap animated, they declare a
            // `Transition` on the underlying stylesheet's color
            // field and the standard `apply_style_to_view` path
            // handles it.
