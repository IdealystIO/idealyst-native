//! Border-routing decision, factored out of `style.rs` so it builds
//! and tests on the host target (the rest of `style.rs` is UIKit-only
//! and compiles to nothing off-device).
//!
//! UIKit gives us two ways to stroke a border, with a sharp split:
//!
...
//!     but each bar is a straight rectangle. With a corner radius the
//!     parent's `clipsToBounds` rounded-corner mask slices the ends off
//!     every bar, leaving notches/gaps at each corner — a straight bar
//!     can't trace a curve.
//!
//! So `apply_style_to_view` routes uniform borders (the common card)
//! through CALayer and reserves the per-side bars for the genuinely
//! asymmetric case CALayer can't represent. This module owns that
//! decision so it can be unit-tested without a device.
...
/// case, where the caller draws straight per-side bars.
...
mod tests {
    ...
    // straight UIView bars, whose corners get sliced by the parent's
    ...
    fn differing_widths_stay_per_side() {
        // A bottom-only border (the per-side feature this change must
        // not regress) must NOT collapse — it needs a single bar.
