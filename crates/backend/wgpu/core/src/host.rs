...
use backend_wgpu_api::{EventSink, SimulatedPlatform};
use crate::backend_impl::WgpuBackend;
use backend_wgpu_api::{Key, KeyEvent, PointerButton, PointerEvent, ScrollEvent};
use crate::keyboard;
use crate::node::{
...
impl Host {
    pub fn new(platform: SimulatedPlatform, color_scheme: ColorScheme) -> Self {
        ...
        // Pre-build keyboard glyph buffers while we have exclusive
        // access to the font system. Cheap (~30 small Buffers); the
        // alternative is mutating the font system from the read-only
        // render walk, which would require a `RefCell` borrow dance
        // on the hot path.
        let keyboard_glyphs = keyboard::build_glyph_cache(&mut font_system.borrow_mut());
    ...
    pub fn tick(&mut self, now: Instant) -> bool {
        ...
        any_anim || any_momentum || kb_alive || caret_alive
    }
    ...
    pub fn pointer_down(&mut self, ev: PointerEvent) {
        ...
        if self.keyboard_visible() {
            ...
            let (kb_rect, hit) =
                keyboard::hit_test(self.platform, self.viewport, slide, ev.position);
