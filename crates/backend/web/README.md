...
backend — most complete primitive coverage and the platform every framework
test and example targets first.
...
- **`batch_queue.rs`** + **`animated.rs`** — the JS-side dispatcher pattern
  ...
  rather than one call per property; the JS dispatcher reads capability flags
  to choose the fastest available update path.
...
1. **Pre-generated cache.** Holds classes minted via `register_stylesheet` —
   variant combinations × theme. Content-keyed and shared across nodes.
   Lifecycle is anchored by the framework's `register_stylesheet` /
...
2. **Dynamic slots, one per styled node.** When a node's resolved style
   ...
   for it. Each styled node owns at most one dynamic class. When the resolved
   style changes:
