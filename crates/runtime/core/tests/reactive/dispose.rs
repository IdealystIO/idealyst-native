...
use runtime_core::{arena_stats, signal};
...
fn scope_owned_signal_freed_by_scope_after_early_dispose_is_safe() {
    ...
    let mut rt = crate::common::TestRuntime::new();
    let owner = rt.render_with(|| {
        let s = runtime_core::signal(10_u32);
        s.dispose(); // early free while the owning scope still lives
        runtime_core::ui! { view { } }
    });
