...
// ---------------------------------------------------------------------------
// Host-registry surface — moved to `runtime-shared`
// ---------------------------------------------------------------------------
//
// Screenshot, VirtualizerCallbacks, ColorScheme, Platform, and the
// per-thread host slots (current platform / color scheme, URL opener,
// full-screen setter, AX announcer) live in `runtime_shared::host` now:
// they are shared substrate (the new core's vocabulary reads the same
// authorities). Re-exported here so every existing `crate::backend::…`
// path keeps resolving; each TLS static has exactly ONE definition —
// runtime-shared's (a second copy would silently split state).
#[allow(unused_imports)] // some names are exercised only by cfg(test)/feature code
pub use runtime_shared::host::{
    announce, color_scheme, install_announcer, install_current_color_scheme,
    install_current_platform, install_fullscreen_setter, install_url_opener, open_url,
    platform, set_fullscreen, ColorScheme, Platform, Screenshot, VirtualizerCallbacks,
};
