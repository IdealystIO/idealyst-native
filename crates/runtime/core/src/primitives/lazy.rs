...
pub fn __dynlink_load(hash: &'static str) -> LazyFuture {
    ...
    match loader {
        ...
        None => Box::pin(async move {
            Err("lazy! dynamic-split: no dynlink loader installed".to_string())
        }),
    }
}
...
mod tests {
    ...
    fn poll_once(mut fut: LazyFuture) -> Option<Result<Element, String>> {
    ...
    #[test]
    fn dynlink_load_without_loader_surfaces_error() {
        // Fresh test thread → no loader installed → the load fails visibly
        // (drives the `.error(..)` UI) instead of silently rendering nothing.
        let r = poll_once(__dynlink_load("deadbeef")).expect("fallback future is ready");
        assert!(r.is_err(), "missing loader is a surfaced error, not an empty view");
    }
    ...
    fn dynlink_load_invokes_installed_loader_with_hash() {
        ...
        assert!(matches!(p, Ok(Element::View { .. })));
    }
}
