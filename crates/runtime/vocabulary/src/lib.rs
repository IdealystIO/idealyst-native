...
pub mod slots;
// The dev-time overlay (`ui-overlay`): the per-node origin tag a patch
// addresses nodes by, and the split-pass version marker. No descriptor
// is compiled in — it is a build artifact. See src/overlay.rs.
