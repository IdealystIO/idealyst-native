...
These suites drive the new core (world → scene → vocabulary handlers)
against **`host-mock`** (`crates/dev/host-mock`): a first-class
`runtime_scene::Host` + all-30-`caps::*Ops` recording mock. They do NOT
use the old `Backend` trait — the historical per-suite
`impl Backend for Mini` + `LegacyBridge` plumbing was retired ahead of
the old-core deletion wave, so this test base survives that deletion.
...
- `caps_conformance.rs` — the per-caps battery: every one of the 30
  ...
  implemented directly" claim. Successor of `bridge.rs`'s role.
- `bridge.rs` — the `LegacyBridge`-specific delegation proof. Still
  written against the old `Backend` trait ON PURPOSE (it tests the
  bridge itself); it is deleted together with `LegacyBridge` in the
  old-core deletion wave.
...
- `walker_ports.rs` — new-core ports of the behavioral coverage from
