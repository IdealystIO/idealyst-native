...
#[doc(hidden)]
pub fn untrack_for_build<R, F: FnOnce() -> R>(f: F) -> R {
    let prev = CURRENT.with(|c| c.borrow_mut().take());
    let result = f();
    CURRENT.with(|c| *c.borrow_mut() = prev);
    result
...
#[doc(hidden)]
pub fn with_scope<R>(scope: &mut Scope, f: impl FnOnce() -> R) -> R {
...
#[doc(hidden)]
pub fn adopt_guard_into_active_scope<G: 'static>(guard: G) -> bool {
    ACTIVE_SCOPE.with(|s| {
        if let Some(&top) = s.borrow().last() {
