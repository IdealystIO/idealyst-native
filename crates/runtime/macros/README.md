...
This crate sits between application code and the runtime. It takes the
...
`runtime_vocabulary::glue`'s primitive constructors (the module
`runtime-core` re-exports as the author surface).
...
3. With `runtime-macros/hot-reload` (forwarded from the host wrapper's
   own `hot-reload` feature), wraps the body through
   `dev_hot::call` so an updated patch swaps the implementation on
...
- `text(...)`, `button(...)`, `view(...)`, `when(...)` for the built-in
  primitive names, which are **snake_case only** (`text`, `button`, `view`,
  `when`, `image`, `icon`, `text_input`, … — see `canonical_primitive` in
  `src/primitives.rs`). A PascalCase tag is always component dispatch.
...
Children flow through the `ChildList` trait, which knows how to flatten an
`Element`, an `Option<T: IntoElement>`, a `Vec<Element>`, or a closure
returning one, into the surrounding slot.
...
The macro does not own the theme; it operates against whatever token set
the consumer hands it via `runtime_core::install_tokens(...)` — which is
what `idea_theme::install_theme(...)` forwards to.
...
- **Reactivity detection is syntactic**: `.get()` calls flag a closure as
  ...
  detected.
- **Primitive dispatch is by literal ident.** `text`, `button`, `view`,
  `when` (and the rest of `canonical_primitive`) are recognized by name
  only; renamed imports won't dispatch.
