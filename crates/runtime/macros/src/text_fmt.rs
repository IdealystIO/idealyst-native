...
//! [`runtime_core::TextSource::JsBinding`] complete with:
//!
...
fn classify(arg: &Expr) -> ArgKind {
    if let Expr::Macro(ExprMacro { mac, .. }) = arg {
        ...
        // write either `bind!(g)` or `runtime_core::bind!(g)` or
...
pub fn emit(input: TextFmtInput) -> TokenStream2 {
    ...
    // Produce a bare `TextSource::JsBinding(...)` value — NOT the
    ...
    //
    //   Text { text_fmt!("c={}", bind!(sig)) }
    //   Button(label = text_fmt!("{}{}", chev, name), ...)
    //
    // The conversion path goes through the `IntoTextSource for
    // TextSource` impl in `sources.rs`, which is identity — zero
    // runtime cost vs the old shape.
    quote! {{
        #(#arg_bindings)*
        ::runtime_core::TextSource::JsBinding(
            ::runtime_core::JsBindingSpec {
