...
//! invocation that registers a `mcp_catalog::ComponentEntry` at module
...
pub(crate) fn emit(
    ...
    let method_submissions = methods.iter().map(|m| {
        ...
        quote! {
            ::runtime_core::__mcp::inventory::submit! {
                ::runtime_core::__mcp::MethodEntry {
    ...
    let animation_submissions = animations.iter().map(|a| {
        ...
        quote! {
            ::runtime_core::__mcp::inventory::submit! {
                ::runtime_core::__mcp::AnimationEntry {
                    parent_module_path: module_path!(),
                    parent_name: #name_str,
                    binding: #binding,
                    initial: #initial,
    ...
    quote! {
        ::runtime_core::__mcp::inventory::submit! {
            ::runtime_core::__mcp::ComponentEntry {
                name: #name_str,
...
/// `runtime_macros::ui!`) are still captured — proc-macros run
