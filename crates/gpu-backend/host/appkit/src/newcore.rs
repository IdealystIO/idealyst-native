//! New-core windowed boot (idea-lite migration, P4a). macOS-only,
//! behind the `new-core` cargo feature.
//!
//! [`run`] is the new-core sibling of [`crate::run`]: identical
//! NSApplication / NSWindow / content-view boot (kept mechanically in
//! sync with `app.rs::run_with` — both go away or merge when the old
//! core is deleted at P7), but the mount goes through
//! `backend_macos::newcore::start` — World + Registry + `realize` +
//! `Backend::finish` + post-mount flush — instead of
//! `runtime_core::mount`. The window's content view is handed to the
//! backend the same way (`create_host_root` → `setContentView` →
//! `set_host_root`), so root attachment is byte-for-byte the old
//! mechanism.
//!
//! Mount buffering: this function opens the buffering window
//! (`begin_mount_buffering`) before calling `start` and closes it after,
//! exactly like the old boot brackets `runtime_core::mount` —
//! `newcore::start` performs the synchronous pre-`finish` drain (see its
//! module docs, step 3), so deferred build microtasks land in the first
//! paint.
...
/// `build()`'s tree on the NEW core, and enter the AppKit run loop.
/// Returns only when the user quits.
///
...
/// `run_with`'s `register_extensions` backend seam).
    ...
    R: FnOnce(&mut SceneRegistry<MacosBackend>),
{
    ...
    // ── Everything down to the mount mirrors app.rs::run_with; see the
    // comments there for the WHY of each step. ─────────────────────────
    ...
    // Scheduler BEFORE anything that schedules; the new-core flush
    // driver rides schedule_microtask/raf_loop, so this is load-bearing
    // for the driver too, not just after_ms.
    backend_apple_core::scheduler::install_scheduler();
    backend_macos::install_render_loop();
    backend_apple_core::log::install_logger();

    // ── Window (IDEALYST_WINDOW_SIZE override, same as app.rs) ────────
    ...
    // frame so the first Taffy pass sees a non-zero viewport (see the
    // app.rs comment for the AppKit sizing timing trap).
    ...
    // navigator closures reach the backend through this — same as the
    // old boot.
    let backend = Rc::new(RefCell::new(backend));
    backend_macos::install_global_self(Rc::downgrade(&backend));

    // ── New-core mount, bracketed by the mount-buffering window ───────
    ...
    // Same retention as the old boot's `forget(owner)`: the app lives
    // until the process exits with the run loop.
