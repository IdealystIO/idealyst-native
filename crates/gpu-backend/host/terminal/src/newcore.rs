//! New-core (idea-lite) entries for the terminal host — the crossterm
//! event loop wired to `backend_terminal::newcore`'s world boot +
//! dispatch-hook flush driver.
//!
//! [`run`] mirrors [`crate::run`] step for step (stderr redirect, panic
//! hook, raw mode, scheduler install, backend construction + global
//! self-handle, cell_size/viewport seeding, the same input/tick/paint
//! frame loop); the only mount-path difference is
//! `backend_terminal::newcore::start` in place of
//! `runtime_core::mount`. The flush discipline needs NO loop changes:
//!
...
//! thread's TLS is intact (the same ordering care the old loop takes
//! with its explicit `Owner` drop).
//!
//! [`render_headless`] is the no-TTY twin of [`crate::render_headless`]
//! for diagnostics and snapshot tests.
...
/// Mount `build` on the new core and render it once, headless — the
/// no-TTY twin of [`crate::render_headless`]. Drives `frames` scheduler
/// ticks between renders so deferred microtasks (and the flush driver's
/// queued flushes) actually run before the snapshot.
...
/// Boot crossterm, mount `build` on the new core, and drive the render
/// loop until the user quits — the new-core leg of [`crate::run`].
/// Restores the terminal state on return.
///
/// `register` runs after `runtime_vocabulary::register_builtins`
/// against the scene registry — the new-core analogue of the old leg's
/// `register_extensions(&mut backend)` hook.
    ...
    R: FnOnce(&mut SceneRegistry<TerminalBackend>),
{
    ...
    // Same stderr + panic-hook posture as the old leg (see crate::run's
    // comments for the why on each).
    ...
    // Scheduler BEFORE mount — the new-core flush driver rides
    ...
    let result = (|| -> Result<(), RunError> {
        loop {
            ...
            // 1. Drain input — identical dispatch to the old loop; the
            //    handlers the backend hands back are the caps-wrapped
            //    closures, so plain `h()` queues the flush.
            while crossterm::event::poll(Duration::from_millis(0))? {
                match crossterm::event::read()? {
                    ...
                    Event::Mouse(MouseEvent {
                        ...
                        // Same borrow discipline as the old loop: the
                        // handler fires AFTER the backend borrow drops.
            ...
            // 3-4. Compose + diff-paint, same as the old loop.
            let grid = backend.borrow_mut().render_to_grid();
            ...
            // 5. Idle strategy, same as the old loop.
