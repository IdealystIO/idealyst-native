...
fn walk<'a>(
    ...
    if !is_native_widget && in_clip {
        ...
        // Painter-driven press feedback may add a background even
        ...
        match &data.kind {
            NodeKind::Text { .. } | NodeKind::Button { .. } => {
                if let Some(entry) = text_store.buffers.get(&data.layout) {
                    ...
                    // Painter-driven press feedback for Buttons:
...
/// [`crate::painter::NavigatorHeaderChrome`] bundle, calls the
/// skin's paint method, and translates each emitted
/// [`crate::painter::NavigatorHeaderHit`] into a
