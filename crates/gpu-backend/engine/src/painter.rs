...
//!
//! A `Painter` is a stateless palette + paint policy: how to draw
//! each native widget chrome (toggle, slider, text input,
//! activity indicator) plus the on-screen keyboard layout and
//! its key chrome. The renderer holds an `Rc<dyn Painter>` and
//! dispatches every per-frame paint call through it.
//!
//! Painters live in their own crates (`ios-sim`, `android-sim`)
...
///
/// Painters are stateless w.r.t. per-widget animation — `t`,
/// `value`, `phase`, focus flags, etc. are passed in by the
/// renderer's walk. Use `Rc<dyn Painter>` to hand a skin to the
/// host; the renderer holds one for the lifetime of the frame.
/// Visual modulation a `Painter` applies to a Button on press.
/// Returned by [`Painter::button_press_visual`] given the current
...
///
/// Painters return [`ButtonPressVisual::rest`] when `t == 0` (or
...
pub trait Painter {
    ...
    /// the recently-tapped key). Painters paint that key with a
    ...
    /// Painters decide button slot positions; the renderer just
    ...
    /// renderer's top-z batch. Painters decide what to draw
...
/// [`Painter::paint_navigator_header`]. Borrows everything by
