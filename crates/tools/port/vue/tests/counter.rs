...
fn counter_fixture_ports_to_expected_rust() {
    let source = fs::read_to_string(fixture("counter.vue")).expect("read counter.vue");
    let (rendered, report) = port_vue::port(&source).expect("port should succeed");
    ...
    assert!(report.holes.iter().any(|h| matches!(h.kind, port_vue::ir::HoleKind::Unsupported)),
        "expected <style> block to be reported as unsupported");
}
