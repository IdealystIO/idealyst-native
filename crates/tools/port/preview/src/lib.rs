...
pub struct PreviewConfig<'a> {
    ...
    /// scratch crate's `Cargo.toml` will reference runtime-core
    /// (and, when rendering, render-wgpu / ios-sim / variant-phone)
...
/// (the directory containing `crates/`), not the runtime-core
/// crate directory.
