...
// The lazy-UI mechanic: the SIDE module constructs a `Primitive` (a View
...
#[no_mangle]
pub extern "C" fn side_make_view() -> *mut Primitive {
    let p: Primitive = view(vec![
        text(format!("hello from side #{}", 7)).into_primitive(),
    ])
    .into_primitive();
