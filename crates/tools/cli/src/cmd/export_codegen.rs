...
/// Generate the umbrella `package.json` so `dist/external` is an
/// installable package (publish it, or `file:`-depend on it from a
/// consumer). Each framework lives in its own **self-contained** folder
/// (`react/`, `vue/`, …, plus the universal `web/`), carrying its own
/// copy of the wasm `pkg/` and custom-element files — nothing reaches
/// across folders. `main`/`types` point at the universal `web/` barrel;
/// deep subpath imports (`react/Greeter.tsx`) resolve into the folders.
/// No `exports` map — those subpath imports stay open.
pub fn gen_package_json(pkg_name: &str, frameworks: &[Framework]) -> String {
    let mut files: Vec<String> =
        frameworks.iter().map(|fw| format!("\"{}\"", fw.slug())).collect();
    ...
    format!(
        "{{\n\
         ...
         \x20 \"version\": \"0.0.1\",\n\
         \x20 \"type\": \"module\",\n\
         \x20 \"main\": \"web/index.js\",\n\
         \x20 \"module\": \"web/index.js\",\n\
         \x20 \"types\": \"web/index.d.ts\",\n\
         \x20 \"sideEffects\": [\"./web/index.js\", \"./*/idl-*.js\"],\n\
         \x20 \"files\": [{files}]\n\
         }}\n",
        files = files.join(", "),
    )
...
impl Framework {
    ...
    /// Parse a `--frameworks` token (case-insensitive; `web`/`js` alias
    /// `vanilla` — the universal no-framework layer). `None` for an
    /// unknown name.
    pub fn parse(s: &str) -> Option<Framework> {
        match s.trim().to_ascii_lowercase().as_str() {
            "vanilla" | "js" | "rawjs" | "web" => Some(Self::Vanilla),
    ...
    /// Output subdirectory / slug (`web`, `react`, `vue`, …). `Vanilla`
    /// maps to `web`: the no-framework helpers live alongside the universal
    /// custom-element + wasm layer in the canonical `web/` folder.
...
mod tests {
    ...
    fn dts_and_react_have_typed_props() {
        ...
        // Self-contained folder: the element file sits in the SAME folder
        // (`react/`), so the import is `./`, not `../`.
    ...
    fn vue_wrapper_lists_props() {
        ...
        // Self-contained folder: element sits beside the wrapper in `vue/`.
    ...
    fn vanilla_helper_create_and_bind() {
        ...
        // Self-contained: the element file is in the same (`web/`) folder.
        assert!(v.contains("import \"./idl-greeter.js\""));
    }
    ...
    fn package_json_and_index_dts() {
        ...
        // The universal layer lives in `web/`; main/types point into it.
        assert!(pkg.contains("\"web\""));
        assert!(pkg.contains("\"main\": \"web/index.js\""));
        assert!(pkg.contains("\"types\": \"web/index.d.ts\""));
        // No stale root-level `pkg`/`index.js` in `files` — each folder is
        // self-contained and carries its own copy.
    ...
    fn framework_parse_and_dispatch() {
        ...
        // Vanilla (no-framework) output lands in the universal `web/` folder.
        assert_eq!(Framework::Vanilla.slug(), "web");
        assert_eq!(Framework::React.slug(), "react");
