...
//!    `docs-app`'s `app()` / `register_scene_extensions()` / `scene_app()`.
//!    It exists so the web
...
"#,
    ...
    // Must match `docs-app`'s real exports: the web wrapper calls
    // `{lib}::app()` and `{lib}::register_scene_extensions` (see
    // `build-web`'s `mount_call`). `register_extensions` was the
    // pre-runtime-v2 name and no longer exists — re-exporting it made
    // the generated crate fail to compile.
    let lib_rs = "//! GENERATED by `idealyst docs` — re-exports the docs-app renderer so the\n\
         ...
         pub use docs_app::{app, register_scene_extensions, scene_app};\n";
...
mod tests {
    ...
    fn generate_docs_app_re_exports_and_is_idempotent() {
        ...
        assert!(
            lib.contains("pub use docs_app::{app, register_scene_extensions, scene_app};"),
            "lib: {lib}"
        );
