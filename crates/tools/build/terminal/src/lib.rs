...
fn runtime_server_main_rs(_bundle_id: &str, bin_name: &str) -> String {
    // No user-crate import — the sidecar owns the user app. The
    // dev-server URL is resolved from the `IDEALYST_DEV_ENDPOINT`
    // env var the CLI sets on the spawned terminal child process.
    format!(
...
fn main() {{
    ...
    if let Err(e) = host_terminal::run_runtime_server(url, opts) {{
        eprintln!("[{bin_name}] runtime error: {{e}}");
        std::process::exit(1);
    }}
}}
