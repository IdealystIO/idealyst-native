...
pub struct HostBinCache {
    ...
    /// map alongside `symbols`. See `dev_hot::diff` for the
...
///
/// Mirrors `dev_hot::diff::strip_mangle_hash` — kept private
/// here rather than depending on `dev-hot` because the host
