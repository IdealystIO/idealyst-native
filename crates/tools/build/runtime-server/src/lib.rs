...
fn generate_sidecar_wrapper(
    ...
    //
    // `hot-reload` puts every `#[component]` in the graph behind the
    // subsecond dispatch split (an outer fn that calls through the jump
    // table plus an inner `__<Name>_hot_impl` carrying the body), and
    // links the substrate that applies a table. Without it the host's
    // patch dylib would link, the table would apply, and every component
    // would keep running the OLD body — the silent failure the
    // `hot_patch: None` in the generated host `main` used to prevent.
    //
    // `runtime-macros` is a proc-macro crate, so this flips the emission
    // for every crate in the sidecar's cargo invocation. Wanted: a
    // half-split tree is a half-patchable app.
    //
    // `ui-overlay` on BOTH sides, and both halves are needed: on
    // `runtime-core` so the user crate's `ui!` sites carry the per-node
    // tags a patch addresses, and on `dev-server` so the sidecar has the
    // applier to honour one. A sidecar with the applier and an app
    // without tags would accept patches and change nothing.
    //
    // Dev-only by construction — this wrapper is only ever generated by
    // `idealyst dev`.
    let runtime_core_dep = source.dep("crates/runtime/core", &["dev", "ui-overlay", "hot-reload"]);
    ...
    let dev_server_dep =
        source.dep(
            "crates/dev/server",
            // `hot-reload` here is the SIDECAR's half of the subsecond
            // tier: harvest the mounted tree's signal values before the
            // re-run and seed them into it. Declared on this dep and
            // not left to feature unification, because dev-server's own
            // `#[cfg(feature = "hot-reload")]` blocks only see its own
            // feature list — without it the patch would apply and every
            // save would reset the app's state.
            &["runtime-server", "screenshot", "ui-overlay", "hot-reload"],
        );
...
"#,
    ...
    let patch_target_dir = sidecar_dir
        ...
        .map(|p| p.join("patches"))
        .unwrap_or_else(|| sidecar_dir.join("patches"));
    // The CLI binary that is generating this wrapper — the same one
    // `cargo_build_fat` points `RUSTC_WRAPPER` at. Resolved here, at
    // generation time, because the host process cannot find it later.
    let idealyst_bin = std::env::current_exe()
        .context("locate the idealyst CLI binary for the host's respawn env")?;
...
/// Bridges the host loop's "give me a `JumpTable` for this crate" to
/// the build-side patch pipeline (replay rustc with `--emit=obj` ->
/// synthesize stubs -> link a dylib -> pair `__*_hot_impl` symbols).
///
/// The sidecar is built with `runtime-core/hot-reload`, so every
/// component body sits behind the jump table and a paired symbol
/// actually rebinds it. If the builder cannot be constructed (no
/// captures, unreadable sidecar symbol table) the host falls back to
/// rebuild-and-respawn, which stays fully live — clients keep their
/// sockets and sessions re-mint on respawn.
...
impl HotPatchAdapter for BuilderAdapter {{
    ...
    fn sidecar_rebuilt(&self, sidecar_bin: &std::path::Path) -> anyhow::Result<()> {{
        self.0.sidecar_rebuilt(sidecar_bin)
    }}
}}
...
fn main() -> std::io::Result<()> {{
    ...
    // Baked in, not discovered: `current_exe()` here is THIS host
    // binary, which has no `rustc-capture` dispatch. A respawn's cargo
    // build must run through the CLI that produced the fat build, or
    // the captures the next patch replays never refresh.
    let idealyst_bin = PathBuf::from("{idealyst_bin}");

    // The patch builder parses the sidecar binary's symbol table once,
    // here, and reuses it for every edit. Failing to construct it is not
    // fatal: the host logs why and rides rebuild-and-respawn, so a dev
    // session still applies saves live either way.
    let hot_patch: Option<Box<dyn HotPatchAdapter>> =
        match HotPatchBuilder::new(captures_dir, &sidecar_path, patch_target_dir, idealyst_bin) {{
            Ok(b) => Some(Box::new(BuilderAdapter(b)) as Box<dyn HotPatchAdapter>),
            Err(e) => {{
                eprintln!(
                    "[runtime-server-host] hot-patch builder unavailable ({{e:#}}) — \
                     falling back to rebuild-and-respawn"
                );
                None
            }}
        }};
...
mod regression_tests {
    ...
    /// The sidecar must enable the dev surface (robot + catalog), the
    /// overlay, AND `hot-reload` — and the host must install the
    /// hot-patch adapter that feeds it.
    ///
    ...
    ///
    /// `hot-reload`: the sidecar and the host are two halves of one
    /// mechanism, and either alone fails SILENTLY — the sidecar without
    /// the split dispatches nothing through the jump table (every save
    /// re-renders the old body); the host without the adapter never
    /// builds a table at all. Both are asserted here so neither can be
    /// dropped on its own.
    #[test]
    fn sidecar_wrapper_enables_dev_overlay_and_hot_reload() {
        ...
        // Both halves of the overlay, and both are load-bearing: the
        // tags come from `runtime-core/ui-overlay` on the USER crate's
        // sites, the applier from `dev-server/ui-overlay` in the
        // sidecar. Either alone is a dev loop that accepts patches and
        // changes nothing.
        assert!(
            facade_feats.iter().any(|f| f == "ui-overlay"),
            "sidecar runtime-core dep must enable `ui-overlay` so the app's \
             `ui!` sites carry tags: {facade_feats:?}",
        );
        let dev_server_feats = dep_features(deps, "dev-server");
        assert!(
            dev_server_feats.iter().any(|f| f == "ui-overlay"),
            "sidecar dev-server dep must enable `ui-overlay` so it can apply \
             a patch: {dev_server_feats:?}",
        );
        ...
        // `hot-reload` is what puts every component body behind the
        // subsecond jump table. Without it on the SIDECAR, the host
        // would build and apply a patch that rebinds nothing and each
        // save would silently re-render the old code.
        assert!(
            facade_feats.iter().any(|f| f == "hot-reload"),
            "sidecar runtime-core dep must enable `hot-reload` (the #[component] \
             dispatch split): {facade_feats:?}",
        );

        let host_main = std::fs::read_to_string(host_dir.join("src/main.rs")).unwrap();
        // The other half of the same pair: the sidecar can dispatch
        // through a table only if the host builds and ships one.
        assert!(
            host_main.contains("HotPatchBuilder::new(captures_dir"),
            "host must construct the hot-patch builder:\n{host_main}",
        );
        assert!(
            host_main.contains("Some(Box::new(BuilderAdapter(b))"),
            "host must install the adapter, not ride the respawn path:\n{host_main}",
        );
        // Construction failure stays non-fatal — respawn is the
        // always-works fallback and a dev session must still come up.
        assert!(
            host_main.contains("falling back to rebuild-and-respawn"),
            "host must degrade to respawn when the builder cannot be built:\n{host_main}",
        );
        // The respawn fallback has to rebuild through the same rustc
        // wrapper the fat build used, and the host cannot find that
        // binary itself (`current_exe()` there is the host). Baked in
        // at generation time; without it every respawn recompiles the
        // world and leaves the captures describing the old source.
        assert!(
            host_main.contains("let idealyst_bin = PathBuf::from(\""),
            "host must bake the CLI path for the respawn env:\n{host_main}",
        );
        assert!(
            host_main.contains("fn rebuild_env") && host_main.contains("fn sidecar_rebuilt"),
            "adapter must forward both respawn hooks:\n{host_main}",
        );
    }
