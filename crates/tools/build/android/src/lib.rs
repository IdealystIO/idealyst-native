...
//! over `backend_android::runtime_server::{{attach, attach_with_url, drain, detach}}`.
...
/// stand up the runtime-server client. Used when the platform's network can
/// see the host's mDNS broadcasts — typical for physical devices on
/// the same Wi-Fi as the dev Mac.
#[no_mangle]
pub extern "system" fn Java_{jni}_NativeBridge_attachRuntimeServer<'local>(
    ...
    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {{
        let app_id_str: String = match env.get_string(&app_id) {{
            Ok(s) => s.into(),
            Err(e) => {{
                log::error!("attachRuntimeServer: bad app_id JString: {{}}", e);
                return;
            }}
        }};
        backend_android::runtime_server::attach(&mut env, context, root, &app_id_str);
    }}));
}}
...
/// resulting URL into manifest meta-data (`IdealystRuntimeServerUrl`). When
/// MainActivity sees that meta-data, it calls this instead of
/// attachRuntimeServer — skipping Bonjour entirely.
#[no_mangle]
