...
fn generate_wrapper(
    ...
    let (cargo_toml, main_rs) = match opts.mode {
        BuildMode::Local => {
            // Old-core wrapper (the embedded-sim host has no new-core
            // CLI leg yet): pin `old-core` for dual-core apps, whose
            // defaults are new-core since the runtime-v2 defaults flip.
            let user_dep = build_ios::old_core_user_dep(project_dir);
            let cargo = format!(
