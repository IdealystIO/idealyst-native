...
fn generate_wrapper(
    ...
    // `new-core` compiles the variant's boot path
    // (`variant_*::run_with` over `host_winit`). The
    // feature is vacuous once the variant/host crates make their
    // contents unconditional; drop it from these lists at that point.
    let form_features: &[&str] = if opts.mode.is_runtime_server() {
        // Native runtime-server shells render streamed wire commands —
        // the wire client is core-agnostic and has no newcore twin.
        &["runtime-server"]
    } else {
        // The variant crates' `new-core` feature is gone — their runtime-v2
        // boot entries (`variant_*::{run, run_at, run_with}`) are the crate's
        // only surface now, so no feature selects them.
        &[]
    };
...
# `variant_<form>::run_with(skin, None, register, build)`.
...
fn main() {{
    ...
    // this is how SDK payload handlers register their per-backend
    // implementations. An unregistered payload panics at realize (the
    // scene contract), so the seam is load-bearing.
    if let Err(e) = {form_ident}::run_with(
        skin,
        None,
...
mod regression_tests {
    ...
    /// The local wrapper mounts through `variant_*::run_with`
    /// (World + scene registry + flush driver) and pins no core feature
    /// on the user crate — there is one core.
    #[test]
    fn local_wrapper_boots_newcore_run_with_plain_user_dep() {
        ...
        assert!(
            ...
            "sim wrapper must take a plain path dep on the user crate:\n{cargo}",
        );
        assert!(
            main_rs.contains("variant_phone::run_with("),
            "sim main must boot through the variant's run_with entry:\n{main_rs}",
        );
        assert!(
            main_rs.contains("register_scene_extensions"),
            "sim main must register through the scene seam:\n{main_rs}",
        );
    }
}
