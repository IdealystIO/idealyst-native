...
impl Host {
    pub fn new(skin: Rc<dyn Skin>, color_scheme: ColorScheme) -> Self {
        ...
        framework_core::set_safe_area_insets(skin.safe_area_insets());
        ...
        if let Some((action, navigator)) = header_action {
            match action {
                ...
                HeaderHitAction::CloseDrawer => {
                    ...
                    {
                        control.dispatch(
                            framework_core::primitives::navigator::NavCommand::CloseDrawer,
                        );
                    }
                }
...
fn label_for_key_event(skin: &dyn Skin, event: &KeyEvent) -> Option<&'static str> {
...
mod tests {
    ...
    //!    primitive through `framework_core::render` reaches the
    ...
    //! [`Skin`] impl. We don't exercise them at unit-test scope; their
    //! state-machine pieces (the active-touches map, the deepest-first
    //! bubble) are implicitly covered by `collect_touch_path` + the
    //! recognizer tests in framework-core.
    use super::*;
    use framework_core::{view, Backend, ColorScheme, TouchResponse};
    use std::cell::Cell;
    ...
    /// runs are cheap.
    /// Minimal `Skin` for headless tests. All paint hooks fall
    ...
    fn make_backend() -> Rc<RefCell<WgpuBackend>> {
        ...
        Rc::new(RefCell::new(WgpuBackend::new(
            ...
            fs,
            ColorScheme::Light,
            Rc::new(TestSkin) as Rc<dyn crate::skin::Skin>,
        )))
    }
    ...
    fn always_consume() -> framework_core::TouchHandler {
