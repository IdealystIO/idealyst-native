...
//!
//! Run with `cargo test -p idealyst-wire`.

use std::rc::Rc;

use framework_core::{Backend, Color, ColorScheme, StyleRules, Tokenized};
use idealyst_dev_client::WireBackend;
use idealyst_dev_server::WireRecordingBackend;
use idealyst_wire::{Command, DevToApp};
...
fn full_round_trip_through_json() {
    ...
    let bytes = idealyst_wire::codec::encode(&envelope).expect("encode");
    ...
    let envelope_decoded: DevToApp =
        idealyst_wire::codec::decode(&bytes).expect("decode");
...
fn event_round_trip_through_handler_table() {
    ...
    let dispatched = recorder
        .dispatch_event(handler_id, idealyst_wire::EventArgs::Unit);
...
fn unknown_node_is_a_protocol_error() {
    ...
    let result = wire_app.apply(Command::Insert {
        parent: idealyst_wire::NodeId(99),
        child: idealyst_wire::NodeId(100),
    });

    assert!(matches!(
        result,
        Err(idealyst_dev_client::ReplayError::UnknownNode(_))
    ));
}
...
fn link_round_trip() {
    ...
    recorder.finish(idealyst_wire::NodeId(1));
...
fn overlay_round_trip() {
    ...
    match create_overlay.0 {
        idealyst_wire::WireOverlayAnchor::Viewport(idealyst_wire::WireViewportPlacement::Bottom) => {}
        other => panic!("expected viewport bottom anchor, got {:?}", other),
    }
    assert!(matches!(
        create_overlay.1,
        idealyst_wire::WireBackdropMode::Dismiss
    ));
...
fn screen_released_reverse_channel() {
    ...
    let callbacks: NavigatorCallbacks<idealyst_wire::NodeId> = NavigatorCallbacks {
        initial_route: "home",
        initial_path: "/",
...
fn color_scheme_helper_maps_correctly() {
    use idealyst_dev_client::color_scheme_to_wire;
    use idealyst_wire::WireColorScheme;
    assert!(matches!(
