//! Compile-checked usage **recipes** for the swap navigator — the canonical
//! "how do I wire a flat, tab-bar-style navigator" example the MCP catalog
//! serves.
//!
//! Why this exists: `swap` replaced the separate `tab` and `drawer`
//! navigators, so there's no `TabNavigator` to reach for — the "tab bar" is
//! now ORDINARY author layout wrapped around the navigator's single outlet.
//! An agent that doesn't know that reconstructs a bespoke bar from scratch (or
//! looks for a tab primitive that no longer exists). This recipe compiles
//! against the live builder API (a signature change that isn't reflected here
//! is a compile error whenever the catalog is built), so the served example
//! can't rot. It mirrors the stack navigator's `stack_two_screens`.
//!
...
recipe!(
    ...
    /// (`.screen`/`.layout`/`.bind`) is an extension trait on
    /// `Bound<SwapHandle>` — it must be in scope or the builder calls fail to
    /// resolve.
    ///
