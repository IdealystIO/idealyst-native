...
/// see the module docs.
#[cfg(all(target_os = "ios", not(target_arch = "wasm32")))]
#[path = "ios.rs"] // module body lives beside lib.rs (pre-P6 layout)
pub mod ios;
#[cfg(all(target_os = "ios", not(target_arch = "wasm32")))]
...
#[cfg(all(target_os = "android", not(target_arch = "wasm32")))]
#[path = "android.rs"] // module body lives beside lib.rs (pre-P6 layout)
pub mod android;
#[cfg(all(target_os = "android", not(target_arch = "wasm32")))]
...
/// Convenience re-exports. Also exports the shared data surface
/// (`Route`, `Screen`) so a same-source app can import everything from
/// here on either core (the `new-core` prelude exports the same names).
pub mod prelude {
    pub use super::{
    ...
    pub use runtime_core::primitives::navigator::{HeaderButton, Route, Screen, StackHeaderState};
}
