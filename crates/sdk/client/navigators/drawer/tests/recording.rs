...
/// Regression: a cold-start deep link to a non-initial drawer item must
/// LAND on that item directly — with no extra navigation and no double
/// mount. A drawer has no back stack, so (unlike the stack handler) there
/// is nothing to reconstruct: the walker resolves the launch URL and hands
/// the deep-linked ABOUT screen straight to `attach_initial`, which is the
/// drawer's body. So exactly one attach, the ABOUT body, no Push/Select.
#[test]
fn recording_drawer_deep_link_lands_on_target_item() {
    dev_server::scheduler::install();
    let mut recorder = WireRecordingBackend::new();
    drawer_navigator::recording::register(&mut recorder);

    // Cold-start launch URL points at the about item (not the initial).
    runtime_core::set_initial_path(Some("/about".to_string()));
    ...
    // The deep-linked ABOUT is the body outlet's initial screen — one
    // attach, no follow-up navigation, no double mount.
    assert_eq!(
        count(&cmds, |c| matches!(c, Command::NavigatorAttachInitial { .. })),
        1,
        "one NavigatorAttachInitial, got: {cmds:?}"
    );
    assert!(has_text(&cmds, "ABOUT BODY"), "deep-linked ABOUT mounted, got: {cmds:?}");
    assert!(!has_text(&cmds, "HOME BODY"), "initial HOME must not also mount, got: {cmds:?}");
    assert_eq!(
        count(&cmds, |c| matches!(c, Command::NavigatorSelect { .. })),
        0,
        "drawer deep link needs no Select (lands via attach_initial), got: {cmds:?}"
    );
    assert_eq!(
        count(&cmds, |c| matches!(c, Command::NavigatorPush { .. })),
        0,
        "drawer deep link must NOT Push, got: {cmds:?}"
    );

    // Root navigator cleared the launch-path slot after mounting.
    assert!(
