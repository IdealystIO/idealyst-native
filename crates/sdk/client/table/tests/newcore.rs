...
//! — exercised against both new-core lowerings:
//!
...
//!   `<table>`/`<tr>`/`<th>`/`<td>` markup — the same DOM the old-core
//!   `Element::External` handlers produced on web.
...
/// flatten them into a single grid (same contract as the old core).
#[test]
...
fn cell_helpers_reach_both_lowerings() {
    ...
    set_cell_interaction(
        &native_cell,
        Rc::new(|_ev| runtime_core::TouchResponse::default()),
        Rc::new(|_entering| {}),
    );
...
/// The scene-Registry handlers must emit genuine table markup — the
/// exact structural contract the old-core `Element::External` web
/// handlers had: `<table>` (with the browser-default reset inline),
...
fn interactive_cell_mounts_through_the_registry() {
    ...
    let page = backend_ssr::newcore::render_path_with("/", table::register_handlers, || {
        ...
        set_cell_interaction(
            ...
            Rc::new(move |_ev| {
                fired.set(true);
                runtime_core::TouchResponse::default()
            }),
            Rc::new(|_| {}),
        );
...
/// The NATIVE grid lowering realized through the new-core scene +
/// vocabulary built-ins must surface its grid props in the emitted CSS
/// — `display: grid` plus a `grid-template-columns` track list. Guards
/// the whole prop path: builder → `StyleProp::Sheet` → `attach_style` →
/// sheet engine → css emission (the "grid props flow through the
/// new-core realization" acceptance).
#[test]
fn native_grid_lowering_emits_grid_css_through_newcore_realization() {
