//! New-core suite: the SAME authored surface — `Markdown(source = …)`
//! (struct-literal `BuildElement` dispatch) and
//! `markdown(src, theme).with_style(…).bind(r)` — mounted through the
//! scene registry on the shared `host-mock` recording substrate.
//!
//! The op-log assertion pins the semantic-DOM shape to the old-core WEB
//! handler (`src/web.rs::build`) call-for-call: outer `<div>` (theme
//! base color/size), per block a semantic element styled then filled
//! with per-run styled text, block inserted into the root AFTER its
//! runs. This is the regression fence for old/new byte-parity — if the
//! handler's emission order or per-run styling drifts, this fails
//! before any live parity diff does.
#![cfg(feature = "new-core")]

use host_mock::Harness;
use markdown::{markdown, Markdown, MarkdownHandle, MdTheme};
use runtime_core::{FontStyle, FontWeight, Length, Ref, StyleRules, Tokenized};
...
fn mounts_the_old_web_handler_dom_shape() {
    ...
    assert_eq!(
        ...
        "create n0 element \"div\"\n\
         ...
         insert n3 <- n5\n\
         insert n0 <- n3",
        "markdown new-core DOM shape drifted from the old-core web handler"
    );
}
...
fn author_style_lands_on_the_outer_div_last() {
    ...
    author.color = Some(Tokenized::Literal(runtime_core::Color("#123456".into())));
...
/// `.bind` fills the `Ref<MarkdownHandle>` at mount (old-core
/// `RefFill::External` timing) with the outer node.
#[test]
...
fn markdown_tag_switch_rebuilds_on_source_change() {
    ...
    // Dedupe-on-equal-scrutinee (the old-core `switch` comment): an
