...
//!   `web` / `android` / `ios` modules (declared in lib.rs). Each one
//!   calls `backend.register_external::<WebViewProps, _>(handler)` to
//!   install a builder closure keyed by `TypeId::of::<WebViewProps>()`.
...
use runtime_core::{Bound, BuildElement, Element, IdealystSchema, Ref, RefFill};
...
/// Interpolate as `{ webview::WebView(WebViewProps { .. }) }`, or use
/// the [`WebView`](type@WebView) tag form directly.
///
...
// ============================================================================
// `ui!` dispatch — type alias + BuildElement impl
//
// The `ui!` macro lowers a PascalCase tag `WebView(url = …)` to a
// struct literal `BuildElement::build(WebView { url: …, .. })`, so the
// tag name must resolve as a *type* with a `BuildElement` impl whose
// `build` returns an `Element`. The fn [`WebView`] (value namespace)
// and this alias (type namespace) coexist — the Card precedent.
// ============================================================================
...
/// in the constructor + `ui!` tag, props struct, handle type, the
/// `.bind(...)` extension trait, and the `url(...)` coercion helper.
pub mod prelude {
...
// Only one is compiled per target via cfg (module decls live in
// lib.rs); this module re-exports both from whichever module matches.
// On targets with no backend support, fallbacks here keep user code
// compiling — the framework's External placeholder is what renders at
// runtime.

#[cfg(target_arch = "wasm32")]
pub use crate::web::register;
#[cfg(target_arch = "wasm32")]
static OPS: &dyn WebViewOps = crate::web::OPS;

#[cfg(all(target_os = "android", not(target_arch = "wasm32")))]
pub use crate::android::register;
#[cfg(all(target_os = "android", not(target_arch = "wasm32")))]
static OPS: &dyn WebViewOps = crate::android::OPS;

#[cfg(all(target_os = "ios", not(target_arch = "wasm32")))]
pub use crate::ios::register;
#[cfg(all(target_os = "ios", not(target_arch = "wasm32")))]
static OPS: &dyn WebViewOps = crate::ios::OPS;
