...
//! `Canvas(CanvasProps { .. }).with_style(…)` — mounted through the
//! scene registry on the shared `host-mock` recording substrate, using
//! the renderer-agnostic SSR/hydration host handler
//! (`register_ssr_scene` — bare `<canvas>` + author style).
//!
...
//! gate + the wave's live web verification; the placeholder arm is
//! pinned in canvas-native's own tests.
...
/// New-core twin of the old-core `unstyled_canvas_defaults_to_fill_parent`
/// regression (the "bare canvas doesn't fill" papercut): an unstyled
/// `Canvas(...)` carries the shared fill-parent sheet, which the handler
/// attaches to the `<canvas>` node.
#[test]
...
/// An explicit `.with_style(...)` REPLACES the fill default (old-core
/// `with_style` semantics) — authors who size the canvas themselves
/// aren't fighting a baked-in 100%.
#[test]
...
fn teardown_releases_the_external_node() {
    ...
    assert!(
        ...
        "unmount must release the canvas node (old walker cleanup-guard parity): {log:?}"
    );
}
