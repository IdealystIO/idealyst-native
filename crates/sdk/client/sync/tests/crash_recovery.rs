...
#[test]
fn cursor_write_lost_after_records_recovers_by_reapply() {
    in_world(async {
...
#[test]
fn cache_write_lost_after_outbox_still_reaches_server() {
    in_world(async {
        let backing: Arc<dyn Storage> = Arc::new(MemoryStorage::new());
        let authority = Rc::new(RefCell::new(Authority::<Item>::new()));
...
/// double-apply.
#[test]
fn outbox_pop_lost_after_ack_does_not_double_apply() {
    in_world(async {
        let backing: Arc<dyn Storage> = Arc::new(MemoryStorage::new());
        let authority = Rc::new(RefCell::new(Authority::<Item>::new()));
...
#[test]
fn offline_create_after_download_reaches_server_on_reconnect() {
    in_world(async {
        let backing: Arc<dyn Storage> = Arc::new(MemoryStorage::new());
        let authority = Rc::new(RefCell::new(Authority::<Item>::new()));
...
#[test]
fn full_offline_edit_then_reconcile_cycle() {
    in_world(async {
        let backing: Arc<dyn Storage> = Arc::new(MemoryStorage::new());
        let authority = Rc::new(RefCell::new(Authority::<Item>::new()));
