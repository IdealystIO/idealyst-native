...
//!   `web` / `android` / `ios` / `macos` modules (declared in lib.rs).
//!   Each one calls `backend.register_external::<VideoProps, _>(handler)`
//!   to install a builder closure keyed by `TypeId::of::<VideoProps>()`.
...
//   - `pub(crate) static OPS: &dyn VideoOps`
// Only one is compiled per target via cfg; this module re-exports
// `register` from whichever module matches. On targets with no backend
...
#[cfg(target_arch = "wasm32")]
static OPS: &dyn VideoOps = crate::web::OPS;
