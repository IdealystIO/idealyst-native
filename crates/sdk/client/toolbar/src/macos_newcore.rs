//! macOS new-core leg — the `MacosBackend`-concrete scene handler.
//!
//! Reuses the core-free NSToolbar machinery in [`crate::macos_shared`]
//! (delegate, toolbar construction, wipe+repopulate item updates, the
//! 0-size placeholder) and contributes only what is new-core-specific:
//!
...
//!   keeps the weakly-held delegate retained, same lifetime model as
//!   the old-core `effect!`), and
...
//!   the new core's wrapped dispatch sites (the External registry
//!   predates the new core), so without this a signal write in the
//!   click handler stays staged forever. This closes the "callbacks
//!   installed by `Element::External` third-party macOS glue … its
//!   port must call `schedule_flush` after author callbacks" residual
//!   named in `backend-macos/src/newcore.rs`'s flush-driver docs.
//!
...
//! live gate for that path is the `toolbar-demo` example run under
//! a new-core boot (`host_appkit::newcore::run_with` + this
//! `register`).
...
use crate::newcore::{finish_mount, ToolbarPrim};

/// Mount handler for `Registry<MacosBackend>` — the old-core
/// `build_toolbar` reproduced over the scene contract: NSToolbar +
/// delegate built and attached to the host window, reactive items via
/// a world effect, 0-size layer-backed placeholder registered with the
/// backend and returned as the in-tree node, then the standard mount
...
pub(crate) fn mount_toolbar_macos(
    ...
    let (mtm, window) = {
        ...
        let mtm = b.mtm();
        // Old handler's posture, unchanged: host_root's window is
        // non-nil by the time handlers fire (`setContentView:` runs
        // before render starts); a windowless mount degrades to a
        // detached toolbar that never displays.
    ...
    // change — the new-core home of the old `effect!` contract. The
    // NativeToolbar clone captured here anchors the delegate (see
    // macos_shared's lifetime model).
    ...
    // Same in-tree node as the old handler: a zero-sized, layer-backed
    // NSView registered with the backend (frame/lifecycle bookkeeping)
    // and returned as `MacosNode::View`.
