//! New-core suite: the SAME authored surface — `form(FormProps { .. })
//! .with_style(…).bind(r)` and the `Form` `ui!` tag shape — mounted
...
//! `external.rs` sequence — the load-bearing contract being that the
//! form's children realize INTO the external node (on web that
...
/// the old walker's `external.rs` create → children order. If children
/// mounted beside the external instead, the web `<form>` would lose
/// autofill + submit-on-enter.
#[test]
fn children_realize_into_the_external_form_node() {
    ...
    assert_eq!(
        log,
        "create n0 external form::newcore::FormProps\n\
         create n1 text \"email\"\n\
         insert n0 <- n1\n\
         create n2 text \"submit\"\n\
         insert n0 <- n2",
        "form mount shape drifted from the old-core External-with-children path"
    );
}

/// Author style attaches to the external node AFTER the children — the
/// old walker's `external.rs` order (create → children → style).
#[test]
fn author_style_lands_on_the_form_node_after_children() {
    ...
    assert!(
        ...
        "author style must attach AFTER the children (old walker order): {log:?}"
    );
}

/// `bind` fills the ref at mount (old-core `RefFill::External` timing);
/// the host fallback ops make `submit()` a documented no-op
...
/// manual impl (table precedent) must route through `form(..)` and
/// yield the same item, children flowing into the external node.
///
