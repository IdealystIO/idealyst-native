...
pub struct WebNavCallbacks<N: Clone + 'static> {
    ...
    /// Initial screen path, RELATIVE to this navigator's [`base`](Self::base).
    /// Composed into the full platform URL via `runtime_core::join_path`
    /// at mount time. For a root navigator `base` is `""`, so the full
    /// path equals this verbatim.
    ...
    /// under (`""` for the root, e.g. `/encounters` for a stack nested
    /// in that drawer screen). Stripped from the platform URL before
    /// resolving; composed back via `join_path` for history ops.
    pub base: String,
    /// Hierarchical deep-link resolver: strips `base`, PREFIX-matches the
    /// navigator's relative routes, and returns the best match
    /// `(route_name, typed_params, remainder)` — `remainder` being the
    /// unconsumed tail a NESTED navigator resolves against its own routes.
    /// Web uses this (not the full-match `match_path`) for cold-load /
    /// deep-link / popstate so a parent navigator can prefix-select a
    /// screen for a deeper URL. On a flat root navigator it behaves like
    /// `match_path` with an empty remainder.
    pub resolve_entry:
...
pub struct NavigatorInstance {
    ...
    /// Exact (full-match) route resolver. Retained for any spot that
    /// needs an exact-URL match; all deep-link / popstate RESOLUTION now
    /// goes through `resolve_entry` (PREFIX match) so a parent navigator
    /// can select a screen for a deeper URL. Kept (not deleted) so the
    /// full-match path stays reachable.
    #[allow(dead_code)]
    match_path: Rc<dyn Fn(&str) -> Option<(&'static str, Box<dyn Any>)>>,
    /// Hierarchical PREFIX resolver (strips `base`, matches relative
    /// routes, returns `(name, params, remainder)`). Used by the
    /// popstate / pop-in-place reconciliation so back navigation to a
    /// deeper URL still prefix-selects this navigator's screen. The
    /// remainder is ignored here — a nested navigator resolves its own
    /// slice from `window.location` independently.
    ...
    /// The portion of the platform URL THIS navigator owns, as of its
    /// last mount/reconcile (see [`Self::owned_of`]). On popstate, if
    /// this navigator's owned slice is unchanged, the URL change belongs
    /// to a NESTED navigator — this navigator must NOT remount its
    /// screen. Remounting would detach + rebuild this navigator's screen
    /// (tearing down the nested navigator's subtree) WHILE the nested
    /// navigator is concurrently popping its own slice, racing a
    /// screen-scoped signal past its dropped scope ("signal used after
    /// its scope was dropped" → "signal type mismatch" → abort).
    /// Initialized empty; set by `mount_internal` on every mount.
    active_owned_url: RefCell<String>,
...
impl NavigatorInstance {
    ...
    /// The portion of `url` THIS navigator owns = the full URL minus
    /// the nested-navigator remainder. `resolve_entry` PREFIX-matches
    /// this navigator's routes and hands back the unconsumed tail
    /// (`remainder`) that a nested navigator resolves against its own
    /// routes; stripping it yields the slice we own. With no remainder
    /// (flat / leaf navigator) the whole URL is ours. Trailing slashes
    /// are normalized off so `/encounters` and `/encounters/` compare
    /// equal (matching `paths_equal`'s tolerance).
    ...
    fn mount_internal(&mut self, name: &'static str, params: Box<dyn Any>, url: String) {
        ...
        // Record the slice of this URL we own so a later popstate can
        // tell whether the change belongs to us or to a nested
        // navigator. Computed from the full `url` (which still carries
        // any nested remainder) before it's moved into the stack entry.
        *self.active_owned_url.borrow_mut() = self.owned_of(&url);
    ...
    fn pop_in_place(&mut self) {
        ...
        // PEEK the entry we'd pop back to BEFORE tearing anything down.
        // If its owned slice equals our currently-active owned slice,
        // this pop wouldn't change THIS navigator's visible screen — the
        // difference lives in a nested navigator's slice. Skipping the
        // detach+remount avoids the same teardown race as `on_popstate`
        // (rebuilding our screen would tear down a nested navigator's
        // subtree out from under it). Only reachable via a programmatic
        // `nav.pop()`, but the guard keeps the two pop paths consistent.
        if let Some(next) = self.url_history.last() {
            if self.owned_of(&next.url) == *self.active_owned_url.borrow() {
                return;
            }
        }
        ...
        // PREFIX-resolve (not full-match) so back to a deeper URL still
        // selects this navigator's screen; the remainder belongs to a
        // nested navigator and is ignored here.
        if let Some((name, params, _remainder)) = (self.resolve_entry)(&entry.url) {
    ...
    fn on_popstate(&mut self) {
        ...
        // If our OWNED slice of the URL is unchanged, the change belongs
        // to a NESTED navigator — do NOT remount our screen. Remounting
        // here would detach + rebuild our screen (tearing down the nested
        // navigator's subtree) while that nested navigator is concurrently
        // popping its own slice, racing a screen-scoped signal past its
        // dropped scope → abort. Let the nested navigator's own popstate
        // handler reconcile its slice. When OUR slice DID change (e.g. a
        // nested stack itself popping `/encounters/abc` → `/encounters`),
        // `owned_of` differs and we fall through to reconcile normally.
        if self.owned_of(&current) == *self.active_owned_url.borrow() {
            return;
        }
        ...
        if let Some(idx) = pos {
            ...
            // PREFIX-resolve so a back to a deeper URL prefix-selects this
            // navigator's screen; the remainder is a nested navigator's job.
            if let Some((name, params, _remainder)) = (self.resolve_entry)(&entry.url) {
                self.mount_internal(name, params, entry.url.clone());
        ...
        let (scroll_x, scroll_y) = self.current_outlet_scroll();
        // PREFIX-resolve a forward navigation to a (possibly deeper) URL.
        if let Some((name, params, _remainder)) = (self.resolve_entry)(&current) {
            if let Some(prev) = self.detach_top() {
                self.url_history.push(HistoryEntry {
                    url: prev.url,
                    scroll_y,
    ...
    F: FnOnce(Rc<RefCell<NavigatorInstance>>),
{
    ...
    let instance = Rc::new(RefCell::new(NavigatorInstance {
        ...
        resolve_entry: callbacks.resolve_entry.clone(),
        depth_changed: callbacks.depth_changed.clone(),
        nav_state: callbacks.nav_state.clone(),
        suppress_popstate: RefCell::new(false),
        // Empty until the first mount; `mount_internal` records the
        // owned slice so popstate can detect "my slice didn't change."
        active_owned_url: RefCell::new(String::new()),
    ...
    let base = callbacks.base.clone();
    let resolve_entry = callbacks.resolve_entry.clone();
    {
        ...
        runtime_core::schedule_microtask(move || {
            ...
            // `initial_path` is RELATIVE to this navigator's base; the
            // platform URL we compare against / write back is the FULL
            // composed path. For a root navigator (`base == ""`) this is
            // `initial_path` verbatim, so flat single-navigator apps are
            // unaffected.
            let full_initial = runtime_core::join_path(&base, initial_path);

            let current = current_pathname();

            if paths_equal(&current, &full_initial) {
                // Plain root mount. Replace state so we own the entry
                // (clears any prior hash/state from page load).
                replace_state(&full_initial);
                inst.mount_internal(initial_route, Box::new(()), full_initial.clone());
            } else if let Some((name, params, _remainder)) = resolve_entry(&current) {
                // Deep link to a non-root screen. PREFIX-resolved so a
                // URL that continues into a NESTED navigator still picks
                // this navigator's screen (the nested navigator resolves
                // the remainder from `window.location` itself — we ignore
                // it here). We want back to return to the home route, so
                // the browser's history gets two entries — but the
                // rendered DOM only shows the deep-linked screen.
                //
                ...
                //   pop can rebuild it later from resolve_entry.
                replace_state(&full_initial);
                if inst.has_layout() {
