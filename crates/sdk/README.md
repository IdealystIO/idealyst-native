...
| `cache` | [`server/cache/`](./server/cache) | Centralized KV cache with TTL — the `Cache` trait (`get`/`set`(+TTL)/`delete` over bytes, `get_json`/`set_json` typed convenience) with `MemoryCache` (always) and `RedisCache` (feature `redis`) backends. Configure at boot via `cache::configure(...)` / `configure_from_env()` (`IDEALYST_CACHE_*`) or the `[cache]` section in `idealyst.toml`; `RedisCache::from_installed()` reuses the app-installed `redis::Client` so one connection serves cache, sessions, rate-limiting, and pubsub. Deliberately excludes atomic read-modify-write — rate limits/counters belong in `server-kit`'s `LimitStore`. |
| `idealyst-config` | [`server/config/`](./server/config) | Unified configuration for the server-tier SDKs. **Named connection profiles** (`[connections.<name>]`) let `jobs`/`pubsub`/`email`/`cache` share one AWS/Redis/Postgres identity — or not — which flat env can't express. `configure_all()` loads + merges `idealyst.toml` (+ per-tool files, `extends` inheritance, env override) and wires every enabled SDK in one call. See [`server/config/README.md`](./server/config/README.md). |
