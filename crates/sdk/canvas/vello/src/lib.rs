...
// NOT available on Apple targets (objc2-foundation 0.3 vs the framework's
// 0.2 conflict) or web (WebGL-only wgpu) — those use `canvas-native`.
#[cfg(all(not(target_arch = "wasm32"), not(target_os = "ios"), not(target_os = "macos")))]
mod render;
#[cfg(all(not(target_arch = "wasm32"), not(target_os = "ios"), not(target_os = "macos")))]
pub use render::register;

/// No-op `register` on targets without vello (Apple + web): vello needs a
/// non-Apple GPU surface; those platforms use `canvas-native`. Still
/// registers the wire serde so a canvas can round-trip over the wire.
