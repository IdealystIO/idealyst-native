...
fn generate_wrapper(
    ...
    let bandroid = workspace_root.join("crates/backend/android");
...
backend-android = {{ path = "{bandroid}", features = ["aas-shell"] }}
jni = "0.21"
log = "0.4"
"#,
            fcore = fcore.display(),
...
//! over `backend_android::aas::{{attach, drain, detach}}`. The dev-host
//! runs the user's reactive tree; this side just feeds incoming
//! wire commands into an `AasClient<AndroidBackend>` on the UI thread.
...
/// stand up the AAS client over the supplied Context + ViewGroup.
/// Called from `MainActivity.onCreate` after the user grants Local
/// Network permission.
#[no_mangle]
