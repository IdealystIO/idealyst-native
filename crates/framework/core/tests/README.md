...
```bash
cargo test -p framework-core --test reactive
```

### One module inside a suite

```bash
cargo test -p framework-core --test reactive topology::
...
```bash
cargo test -p framework-core --test reactive topology::diamond
```
...
cargo test -p framework-core --features async-driver
cargo test -p framework-core --features robot
cargo test -p framework-core --features debug-stats
cargo test -p framework-core --features hot-reload

# Everything together
cargo test -p framework-core --all-features
```
...
cargo llvm-cov --html --branch -p framework-core --all-features

# Just the number, for CI
cargo llvm-cov --branch --summary-only -p framework-core --all-features

# Mutation testing (slow — run nightly, not per-commit)
cargo mutants -p framework-core
```
...
4. Run `cargo test -p framework-core --test <name>` to verify.
