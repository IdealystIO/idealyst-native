...
(`framework-macros`, `idea-ui`, the backends) or routed through it (host
crates, `dev-client`, `wire`). Application code depends only on
`framework-core` plus the macros — backends are named in the host crate, not
in app code.
...
- **`Primitive` enum** — the closed set of "things the render walker knows
  ...
  Adding a new primitive means a new variant plus the `Backend` hooks that go
  with it.
- **`Backend` trait** — `crates/framework/core/src/backend.rs`. The seam
  ...
  variants, navigator chrome, `set_animated_*`, etc.). Optional hooks default
  to `unimplemented!()` so a backend that hasn't wired a primitive panics
  loudly rather than silently no-oping. See `docs/backend.md` for the long
  version.
...
- **Styles + theming** — `style.rs`. `StyleRules`, the `stylesheet!`
  ...
  signals). The author-facing macro is in `framework-macros`; this crate
...
- **Mount entry point** — `framework_core::mount(backend, app)`. **Host
...
- **`hot-reload`** — wires the `framework-hot` substrate into the walker
