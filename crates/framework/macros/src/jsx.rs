...
fn emit_when(props: &[&Prop], _children: Option<&[JsxNode]>) -> TokenStream2 {
    ...
    let then_e = props
        ...
        .find(|p| p.name == "then")
        .map(|p| emit_attr_value_raw(&p.value))
        .unwrap_or_else(|| quote! { || ::framework_core::view(::std::vec::Vec::new()) });
    let other = props
        .iter()
...
mod tests {
    ...
    fn ref_attr_emits_bind_call() {
        ...
        assert!(out.contains(":: framework_core :: button"));
    ...
    fn multiple_top_level_elements_wrap_in_view() {
        ...
        assert!(out.contains(":: framework_core :: view"));
        let count = out.matches(":: framework_core :: text").count();
        assert_eq!(count, 2);
    }
    ...
    fn fragment_inside_element_flattens_via_childlist() {
        ...
        // the fragment.
        let view_count = out.matches(":: framework_core :: view").count();
        assert_eq!(view_count, 1, "expected exactly one view emission, got: {out}");
        let text_count = out.matches(":: framework_core :: text").count();
