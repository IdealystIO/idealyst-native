...
//!   `IntoOverrideSource` traits in framework-core.
...
fn transition_value_expr(t: &TransitionDecl) -> TokenStream2 {
    ...
    let easing = if let Some((a, b, c, d)) = &t.cubic_bezier {
        quote! { ::framework_core::Easing::CubicBezier(#a as f32, #b as f32, #c as f32, #d as f32) }
    } else {
        let variant = &t.easing;
        quote! { ::framework_core::Easing::#variant }
    };
    quote! {
        ::framework_core::Transition::new(#duration, #easing)
    }
}
...
fn emit_variant_enum(decl: &StyleSheetDecl, axis: &VariantAxisDecl) -> TokenStream2 {
    ...
    quote! {
        ...
        impl ::framework_core::VariantEnum for #enum_name {
...
fn emit_builder(decl: &StyleSheetDecl) -> TokenStream2 {
    ...
    let axis_setters = decl.variants.iter().map(|axis| {
        ...
        let enum_name = format_ident!("{}{}", decl.name, pascal(&axis.axis));
        quote! {
            pub fn #setter<V: ::framework_core::IntoVariantSource<#enum_name>>(mut self, value: V) -> Self {
    ...
    let override_setters = decl.overrides.iter().map(|o| {
        ...
        let ty = &o.ty;
        quote! {
            pub fn #setter<V: ::framework_core::IntoOverrideSource<#ty>>(mut self, value: V) -> Self {
    ...
    quote! {
        ...
        impl #name {
            ...
            pub fn sheet() -> ::std::rc::Rc<::framework_core::StyleSheet> {
                #stylesheet_fn()
            }
