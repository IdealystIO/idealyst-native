...
//!
//! Lives in this crate (not `framework-core`) because `PrimitiveEntry`'s
...
//! `framework-core::primitive` is caught by:
...
inventory::submit! {
    PrimitiveEntry {
        ...
        docs: "GPU-rendered drawing surface. Local-render only — emits a placeholder under AAS (see [[aas_graphics_unsupported]]).",
        props: &[
            COMMON_STYLE_FIELD,
            COMMON_REF_FILL_FIELD,
            COMMON_ACCESSIBILITY_FIELD,
