...
`idealyst new` scaffolds a library crate plus a `.claude/mcp.json` that wires Claude Code into the project's MCP server. `idealyst dev` builds the AAS host binary, launches it, and serves the wasm bundle so you can see the app in a browser while the native targets stream the same wire log.
