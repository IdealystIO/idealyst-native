//! New-core leg of `wire_virtualizer.rs`: a scene containing a
//! virtualizer, realized through the recorder's caps adoption
//! (`caps::VirtualizerOps::create_virtualizer` → the same `Backend`
//! emission), must put `Command::CreateVirtualizer` on the wire with
//! the snapshotted keys/sizes — not panic, and not silently drop the
//! list.
//!
//! Like the old-core twin, this test records WITHOUT replaying into a
//! client: `dev-client`'s `apply_create_virtualizer` is still the
//! documented lazy-mount stub (it registers no node, so replaying a
//! virtualizer insert errors `UnknownNode`) — a pre-existing gap shared
//! by both cores, not a new-core regression.

use runtime_core::primitives::virtualizer::ItemSize;
use runtime_vocabulary::builders::{text, view, virtualizer};

#[test]
fn newcore_virtualizer_emits_create_virtualizer_with_snapshot() {
    ...
    // Before the recorder's caps adoption existed, realizing this scene
    // would have had no `VirtualizerOps` target at all; the assertion
    // below is the new-core mirror of the old walker regression.
