// End-to-end catalog-emission suite — **shared by both cores**.
//
// A `#[component]`-annotated function in this file should appear in
// `catalog::entries()` at runtime, with its doc comment captured and
// its `composes` edges extracted from any `ui!` / `jsx!` invocations in
// the body. Validates the full emission + `inventory` distributed-slice
// round-trip.
//
...
//
// # Why this file is `include!`d rather than a test target
//
// The catalog has **two** inventory anchors, one per lowering (see
// `runtime_core::__mcp` / `runtime_vocabulary::glue::__mcp`), and only
// one of them used to be tested. Proving they emit the same inventory
// means compiling this SAME source under both lowerings:
//
// - old core: `crates/mcp/catalog/tests/registers_component.rs`
//   (`cargo test -p mcp-catalog`) — `runtime_core` is the real crate,
//   `#[component]` emits `::runtime_core::__mcp`. This is the leg that
//   disappears with `runtime-core`, which is why the shared body lives
//   on the new-core side.
// - new core: `crates/dev/newcore-catalog/tests/registers_component.rs`
//   (`cargo test -p newcore-catalog`) — `extern crate runtime_core as
//   runtime_core;`, `#[component]`'s emission is retargeted to
//   `::runtime_vocabulary::glue::__mcp` while the NON-retargeted
//   emissions (`#[derive(IdealystSchema)]`, `#[idealyst_tool]`,
//   `recipe!`, `doc_scope!`) resolve `::runtime_core::__mcp` through the
//   facade root. Both anchors re-export the same `mcp-catalog`
//   instance, so a single inventory must result — pinned by
//   [`catalog_inventory_is_identical_across_cores`], whose expected
//   value is a literal in this shared source and therefore asserted
//   verbatim on both legs.
//
// mcp-catalog cannot host the new-core leg itself: `runtime-core`
// depends (transitively) on `mcp-catalog`, so a non-dev dep would be a
// cargo cycle, and a *dev*-dep can't be optional — an unconditional one
// would enable `runtime-macros/new-core` for the old leg too
// (proc-macro features unify graph-wide).
//
// Everything in here is core-agnostic: `runtime_core::…` is either the
// real old core or the facade alias, and the catalog surface is reached
// through the `__mcp` anchor rather than a direct `mcp-catalog` dep (the
// new-core leg only sees the crate through the anchor).

use runtime_core::{
    component, idealyst_tool, jsx, recipe, ui, ChildList, Element, IdealystSchema, IntoElement,
};

/// The catalog crate, reached through whichever core is in scope — this
/// is the anchor the emissions themselves resolve.
use runtime_core::__mcp as catalog;

/// Stub body for components whose only job is to register. Same source
/// on both cores: `view(children)` returns the old `Bound<ViewHandle>`
/// or the glue `GlueView`, and `IntoElement` is implemented for both.
///
/// Deliberately NOT a `ui!` body — `democomponent` asserts an EMPTY
/// `composes` list, so the stub must contribute no component-position
/// idents.
fn stub_view() -> Element {
    IntoElement::into_element(runtime_core::view(::std::vec::Vec::new()))
}
...
fn doc_scope_registers_with_slug_and_title() {
    let demo = catalog::lookup_scope("demo").expect("demo scope registered");
    ...
    let inner = catalog::lookup_scope("inner-feature")
...
fn scope_for_picks_nearest_enclosing_scope() {
    let cat = catalog::ResolvedCatalog::build();
...
fn primitives_table_includes_core_set() {
    let names: Vec<&str> = catalog::primitives().map(|p| p.name).collect();
...
fn states_table_has_exactly_the_four_interaction_states() {
    let mut names: Vec<&str> = catalog::states().map(|s| s.name).collect();
    names.sort();
...
fn utilities_table_includes_platform_accessor() {
    let utils: Vec<&catalog::UtilityEntry> = catalog::utilities().collect();
    let platform = utils
        .iter()
...
fn macros_table_documents_effect_and_signal() {
    ...
    let effect = catalog::lookup_macro("effect").expect("`effect!` macro registered");
    assert_eq!(effect.module_path, "runtime_core");
    assert_eq!(effect.kind, catalog::MacroKind::Reactive);
    assert!(
    ...
    assert!(catalog::lookup_macro("effect!").is_some());
    ...
    assert!(catalog::lookup_macro("signal").is_none(), "signal! macro was removed");
    let sig = catalog::lookup_utility("signal").expect("`signal` fn documented as utility");
    assert_eq!(sig.module_path, "runtime_core");
    assert_eq!(sig.category, catalog::UtilityCategory::Reactive);
    assert!(
    ...
    let component = catalog::lookup_macro("component").expect("`#[component]` registered");
    assert_eq!(component.invocation, "#[component]");
    assert_eq!(component.kind, catalog::MacroKind::Component);
}
...
fn sdks_table_indexes_non_ui_capability_crates() {
    ...
    let by_name = |needle: &str| {
        catalog::sdks()
            ...
            .unwrap_or_else(|| panic!("`{needle}` SDK registered; got {:?}",
                catalog::sdks().map(|s| s.name).collect::<Vec<_>>()))
    };

    let net = by_name("net");
    assert_eq!(net.category, catalog::SdkCategory::Data);
    assert_eq!(net.kind, catalog::SdkKind::Api);
    ...
    for required in &["storage", "credentials", "server", "files"] {
        ...
        assert_eq!(s.category, catalog::SdkCategory::Data, "{required} is a data crate");
    }
...
fn sdks_guide_enumerates_the_data_crates() {
    ...
    let srv = catalog::lookup_guide("server-functions")
...
fn primitive_entry_is_construct_locked_to_this_crate() {
    ...
    let view = catalog::lookup_primitive("view").expect("view primitive registered");
...
fn resolver_links_ui_host_to_actual_child_entries() {
    ...
    let users = cat.uses(&catalog::EntryRef::of(child_a));
    assert!(
...
fn props_schema_records_fields_with_docs_and_constraints() {
    let s = catalog::lookup_schema("BadgeProps")
...
fn cross_module_resolution_picks_same_module() {
    ...
    let root_edges = cat.dependencies(&catalog::EntryRef::of(root_host));
    let resolved = match &root_edges[0].status {
        catalog::EdgeStatus::Resolved { target } => *target,
    ...
    let sub_edges = cat.dependencies(&catalog::EntryRef::of(sub_host));
...
fn empty_catalog_builds() {
    let cat = catalog::ResolvedCatalog::build_from(Vec::new());
    assert!(cat.entries().is_empty());
}

fn find_entry(name: &str) -> &'static catalog::ComponentEntry {
    catalog::entries()
        .find(|e| e.name == name)
...
fn enum_schema_emits_type_entry_with_variants() {
    let t = catalog::lookup_type("DemoSize").expect("DemoSize TypeEntry registered");
    let variants = match &t.shape {
        catalog::TypeShape::Enum { variants } => *variants,
...
///
/// **Inline props, not `props: &CounterProps`** — deliberately: the
...
fn methods_block_emits_method_entries() {
    let methods: Vec<&catalog::MethodEntry> = catalog::methods()
...
fn animated_macros_emit_animation_entries() {
    let anims: Vec<&catalog::AnimationEntry> = catalog::animations()
...
fn catalog_json_round_trips_through_build_from_json() {
    use catalog::ResolvedCatalog;

    // `Value: Display` renders JSON, so the round-trip needs no
    // serde_json dep of its own (the new-core leg reaches the catalog
    // only through the `__mcp` anchor).
    let json_str = catalog::catalog_json().to_string();
    ...
    assert_eq!(rebuilt_net.category, catalog::SdkCategory::Data, "sdk category lost");
    assert_eq!(rebuilt_net.kind, catalog::SdkKind::Api, "sdk kind lost");
    ...
    let direct_enum_variants: usize = direct
        ...
        .filter_map(|t| match &t.shape {
            catalog::TypeShape::Enum { variants } => Some(variants.len()),
            _ => None,
        })
        .sum();
...
// -----------------------------------------------------------------------------
// Cross-core inventory identity.
//
// The two cores emit through DIFFERENT anchors (`runtime_core::__mcp` vs
// `runtime_vocabulary::glue::__mcp`, plus the facade root for the
// non-retargeted derive/tool/recipe/scope emissions). This suite compiles
// from one shared source under both lowerings, so the resulting inventory
// must be identical — and because the expected value below is a literal in
// that shared source, both legs assert the SAME string. A retarget that
// dropped, duplicated, or reshaped any emission moves this fingerprint on
// exactly one leg.
//
// Deliberately excluded from the fingerprint: `file` and `line`. `file!()`
// reports the path by which this file was `include!`d, which differs
// between the two host crates (relative vs. CARGO_MANIFEST_DIR-anchored);
// their non-emptiness is asserted by
// `democomponent_registers_in_catalog`.
// -----------------------------------------------------------------------------

/// Every macro-EMITTED slice, rendered as sorted, stable lines. The
/// framework's hand-curated static tables (primitives / utilities /
/// macros / states / guides / sdks / icon_sets) live inside the catalog
/// crate itself and are core-independent by construction, so they are
/// covered by the per-slice tests above rather than pinned here (adding a
/// primitive must not have to touch this literal).
fn emitted_inventory_fingerprint() -> String {
    fn params(ps: &[catalog::ParamSpec]) -> String {
        ps.iter()
            .map(|p| format!("{}:{}", p.name, p.type_short_name))
            .collect::<Vec<_>>()
            .join(",")
    }
    fn fields(fs: &[catalog::PropFieldSpec]) -> String {
    ...
    for e in cat.entries() {
        let composes = e
            ...
            .join(",");
        lines.push(format!(
            "component {}::{} params=[{}] composes=[{}] docs={}",
            e.module_path,
            e.name,
            params(e.params),
            composes,
            e.docs.trim().is_empty() as u8,
        ));
    }
    for s in cat.scopes() {
        lines.push(format!(
            "scope {} title={:?} order={} module={}",
            s.slug, s.title, s.order, s.module_path
        ));
    }
    for t in cat.types() {
        let shape = match &t.shape {
            catalog::TypeShape::Struct { fields: f } => format!("struct[{}]", fields(f)),
            catalog::TypeShape::Enum { variants } => format!(
                "enum[{}]",
                variants
                    .iter()
                    .map(|v| format!("{}({})", v.name, fields(v.payload)))
                    .collect::<Vec<_>>()
                    .join(",")
            ),
        };
    ...
    for r in cat.recipes() {
        lines.push(format!(
            "recipe {} target={} uses=[{}] source_bytes={}",
            r.name,
            r.target,
            r.uses.join(","),
            r.source.len()
...
/// The pinned inventory, one line per emitted entry, sorted. Regenerate
/// ONLY when an emission genuinely changed, and only after confirming
/// every leg produces the new value — a diff here that appears on ONE
/// leg is a retarget bug, not a stale literal.
const EXPECTED_EMITTED_INVENTORY: &[&str] = &[
    "animation registers_component::fader_demo::_opacity initial=0.0_f32",
    "animation registers_component::fader_demo::_scale initial=1.0_f32",
    "component registers_component::Ambiguousname params=[] composes=[] docs=0",
    "component registers_component::ChildA params=[] composes=[] docs=1",
    "component registers_component::ChildB params=[] composes=[] docs=1",
    "component registers_component::ChildC params=[] composes=[] docs=1",
    "component registers_component::Nest params=[props:NestProps] composes=[view] docs=1",
    "component registers_component::badge_host params=[_props:BadgeProps] composes=[] docs=0",
    "component registers_component::counter params=[initial:Reactive,bind_to:Option] composes=[] docs=0",
    "component registers_component::democomponent params=[_props:DemoProps] composes=[] docs=0",
    "component registers_component::docless params=[] composes=[] docs=1",
    "component registers_component::fader_demo params=[] composes=[] docs=1",
    "component registers_component::for_host params=[] composes=[view,ChildA] docs=0",
    "component registers_component::jsx_fragmented params=[] composes=[] docs=1",
    "component registers_component::jsx_host params=[] composes=[jsx_outer,jsx_inner,jsx_fragmented] docs=0",
    "component registers_component::jsx_inner params=[] composes=[] docs=1",
    "component registers_component::jsx_outer params=[props:JsxOuterProps] composes=[view] docs=1",
    "component registers_component::multi_ui_host params=[] composes=[ChildA,ChildB] docs=0",
    "component registers_component::multiline_docs params=[] composes=[] docs=0",
    "component registers_component::nested_host params=[] composes=[Nest,ChildA,ChildB,ChildC] docs=0",
    "component registers_component::positional_host params=[idx:Reactive,_label:str] composes=[] docs=0",
    "component registers_component::root_host_with_dupe params=[] composes=[Ambiguousname] docs=1",
    "component registers_component::scoped::DemoLeaf params=[] composes=[] docs=1",
    "component registers_component::scoped::inner::InnerLeaf params=[] composes=[] docs=1",
    "component registers_component::stmt_macro_host params=[] composes=[ChildA] docs=0",
    "component registers_component::submodule::Ambiguousname params=[] composes=[] docs=1",
    "component registers_component::submodule::submodule_host params=[] composes=[Ambiguousname] docs=1",
    "component registers_component::ui_host params=[] composes=[ChildA,ChildB,ChildC] docs=0",
    "method registers_component::counter::bump_by params=[n:i32]",
    "method registers_component::counter::reset params=[]",
    "recipe animated_toast target=presence uses=[button,text,view] source_bytes=2263",
    "recipe compute_thing_example target=compute_thing uses=[] source_bytes=127",
    "recipe confirm_dialog_overlay target=overlay uses=[button,overlay,text,view] source_bytes=3017",
    "recipe input_with_submit target=text_input uses=[button,text,text_input,view] source_bytes=1723",
    "recipe keyed_list_add_remove target=ui uses=[button,text,view] source_bytes=1752",
    "recipe stack_two_screens target=StackNavigator uses=[button,text,view] source_bytes=6234",
    "recipe swap_three_screens_tab_bar target=SwapNavigator uses=[button,text,view] source_bytes=5113",
    "schema registers_component::BadgeProps [label,count,color]",
    "scope core title=\"Core\" order=0 module=runtime_core",
    "scope demo title=\"Demo Feature\" order=5 module=registers_component::scoped",
    "scope inner-feature title=\"Inner Feature\" order=0 module=registers_component::scoped::inner",
...
#[test]
fn catalog_inventory_is_identical_across_cores() {
    let actual = emitted_inventory_fingerprint();
    assert_eq!(
        actual,
        EXPECTED_EMITTED_INVENTORY.join("\n"),
        "\n--- catalog emission inventory drifted ---\nIf this fails on ONE core only, the \
         retarget lost/gained an emission — fix the lowering, not this literal.\nactual:\n{actual}\n"
    );
}
