...
pub fn serve(
    ...
    serve_with_tick_and_port(addr, recorder, app_id, || {}, None, None)
}
...
pub fn serve_with_port_mirror(
    ...
    serve_with_tick_and_port(addr, recorder, app_id, || {}, Some(port_mirror), None)
}

/// Same as [`serve_with_port_mirror`] but also forwards every
/// inbound `AppToDev` event to the sidecar held in `sidecar_slot`
/// instead of (or in addition to) dispatching it locally. Used by
/// the split-process AAS host where the user's reactive runtime
/// lives in a sidecar child and the WS-server here is purely a
/// transport — no handler closures are registered on the local
/// `recorder`, so a local `dispatch_event` would no-op.
...
where
    F: FnMut(),
{
    serve_with_tick_and_port(addr, recorder, app_id, on_tick, None, None)
}
    ...
    F: FnMut(),
{
    ...
    loop {
        ...
        poll_reads(&mut clients, &recorder, sidecar_slot.as_ref());
        drain_sidecar_inbound(&recorder, sidecar_slot.as_ref());
...
/// Pull any new `SidecarOut::Commands` frames the sidecar has
/// produced and append them to the recorder. Runs on the same thread
/// as the rest of the serve loop because `WireRecordingBackend` is
/// `!Send` (Rc-based) — the reader thread can only ferry frames;
/// applying them must happen here.
        ...
        loop {
            match client.ws.read() {
                Ok(Message::Text(t)) => match serde_json::from_str::<AppToDev>(t.as_str()) {
                    Ok(msg) => handle_app_msg(recorder, msg, sidecar_slot),
...
fn handle_app_msg(
    ...
    // Hello is handled locally regardless — it's a transport-layer
    // greeting, not a reactive event. Forward everything else to the
    // sidecar when one is configured (split-process mode), since the
    // local recorder has no registered handler closures in that case
    // and would silently no-op.
    if let AppToDev::Hello { ref app_name, .. } = msg {
    ...
    if let Some(slot) = sidecar_slot {
        if let Ok(guard) = slot.lock() {
            if let Some(sidecar) = guard.as_ref() {
                sidecar.send(crate::SidecarIn::Event(msg));
                return;
            }
        }
        // Sidecar slot exists but is empty (rebuild in flight). Drop
        // the event — the user can retry, and the new sidecar's
        // initial render likely renders a fresh tree anyway.
        return;
    }

    // No sidecar — single-process mode. Dispatch locally.
    match msg {
        AppToDev::Hello { .. } => {} // unreachable; handled above
