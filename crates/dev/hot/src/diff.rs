...
//!    pointer so subsequent `dev_hot::call` sites dispatch
//!    to the patched code.
//!
//! ## Errors
//!
...
pub fn apply_from_dylib(dylib_path: &Path) -> Result<(), DiffError> {
    ...
    if bin_map.is_empty() {
        return Err(DiffError::Parse(format!(
            "bin {} has no hot symbols (was the host built with runtime-core/hot-reload?)",
            bin_path.display(),
        )));
    }
    ...
    // built as a bin (which is what `dx serve` does); our runtime-server
    ...
    eprintln!(
        "[dev-hot] jump-table: {} entries (bin syms: {}, dylib syms: {}) — aslr_ref(bin)=0x{:x} new_base(dylib)=0x{:x}",
        map.len(),
        bin_map.len(),
        dylib_map.len(),
        aslr_reference,
    ...
    for (k, v) in map.iter().take(5) {
        eprintln!("[dev-hot]   entry: 0x{:x} -> 0x{:x}", k, v);
    }
    ...
    eprintln!(
        "[dev-hot] apply breakdown: bin_syms {}ms, dylib_syms {}ms, subsecond apply {}ms, total {}ms",
        bin_ms,
        dylib_ms,
        apply_ms,
