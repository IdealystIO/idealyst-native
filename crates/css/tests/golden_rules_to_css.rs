...
/// The text-primitive form is IDENTICAL except the shadow lowers to
/// `text-shadow` — deriving the expectation from [`GOLDEN_BOX`] encodes
/// exactly that contract (any other divergence still fails).
#[test]
fn golden_full_struct_text_shadow() {
    let expected = GOLDEN_BOX.replacen("box-shadow: ", "text-shadow: ", 1);
    assert_eq!(css::rules_to_css_text(&maximal_rules()), expected);
}
