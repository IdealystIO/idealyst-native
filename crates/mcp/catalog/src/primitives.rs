...
inventory::submit! {
    PrimitiveEntry {
        ...
        docs: "Scrollable container. Backed by `UIScrollView` (iOS), `ScrollView` (Android), CSS `overflow: auto` (web), `NSScrollView` (macOS). Preserves scroll position across layout passes (see [[ios_scrollview_bounds_origin]]). Sized as a viewport: it fills the available main-axis space of its parent rather than growing to its content — bound it with `max_height`/`max_width` (a bare `height` is shadowed by the flex seed, as in CSS).",
        props: &[
            PropFieldSpec {
                name: "children",
                type_str: "Vec<Element>",
            ...
            PropFieldSpec {
                name: "horizontal",
                type_str: "bool",
                doc: "Scroll on the x axis instead of the y axis. Children of a horizontal scroller are exempt from the default `max-width: 100%` clamp so the content can exceed the viewport.",
                constraint: "",
            },
            PropFieldSpec {
                name: "on_scroll",
                type_str: "Fn(f32, f32)",
                doc: "Content offset `(x, y)` in logical px, delivered per scroll event. Reports WHERE the reader is; it cannot say how much is left — for that use `on_end_reached`.",
                constraint: "inline prop or builder method `.on_scroll(..)`",
            },
            PropFieldSpec {
                name: "on_end_reached",
                type_str: "Fn()",
                doc: "Called once when the reader arrives within `end_reached_threshold` of the end of the scroll axis — edge-triggered (not per event), re-armed when they leave, never for content that fits its viewport. The load-on-scroll / infinite-paging hook. Answered on iOS, web and macOS; a no-op on Android, Linux and Windows, where a list that only grows this way stops growing.",
                constraint: "inline prop or builder method `.on_end_reached(..)`; backends: ios, web, macos",
            },
            PropFieldSpec {
                name: "end_reached_threshold",
                type_str: "f32",
                doc: "How close to the end counts as arriving, in logical px. `0` (the default) is the very end; a screenful is the usual choice for prefetching so the next page is there before the reader is.",
                constraint: "",
            },
            PropFieldSpec {
                name: "bounces",
                type_str: "bool",
                doc: "May the scroller travel past its content? `false` stops the rubber-band on iOS and, on web, scroll chaining into the page behind it (`overscroll-behavior` on the scroller's own axis). Default: the platform's.",
                constraint: "",
            },
            PropFieldSpec {
                name: "always_bounce",
...
inventory::submit! {
    PrimitiveEntry {
        ...
        docs: "Recycled-row list primitive. Renders only the items currently in the viewport; intended for long lists where `scroll_view` over `for` would blow up the view tree. The author-facing spelling is `flat_list(data = …, render = …)` in `ui!`, or `flat_list(..)` from `runtime_core::primitives::flat_list`. Sized as a viewport: it fills its parent's main axis — bound it with `max_height`, not a bare `height`.",
        props: &[
            PropFieldSpec {
                name: "data",
                ...
                constraint: "`flat_list` spelling; the raw builder takes `item_count` / `item_key` closures",
            },
            PropFieldSpec {
                name: "render",
            ...
            PropFieldSpec {
                name: "size",
                type_str: "FlatListItemSize<T>",
                doc: "Row extent along the scroll axis: `fixed_size(px)` or a per-item closure. Default: `fixed_size(48.0)`.",
                constraint: "",
            },
            PropFieldSpec {
                name: "axis",
                type_str: "Axis",
                doc: "Scroll direction. Default vertical.",
                constraint: "",
            },
            PropFieldSpec {
                name: "lanes",
                type_str: "Lanes",
                doc: "Cross-axis lanes for a uniform grid (`Lanes::Fixed(n)`). Default one lane — a plain list.",
                constraint: "",
            },
            PropFieldSpec {
                name: "gap",
                type_str: "f32",
                doc: "Spacing between rows and between lanes, in logical px. `main_spacing` / `cross_spacing` set the two independently.",
                constraint: "",
            },
            PropFieldSpec {
                name: "overscan",
                type_str: "f32",
                doc: "Extra viewport-fractions to lay out off-screen in each direction so a fling has rows ready. Default backend-chosen.",
                constraint: "",
            },
            PropFieldSpec {
                name: "on_scroll",
            ...
            PropFieldSpec {
                name: "on_end_reached",
                type_str: "Fn()",
                doc: "Called once when the reader arrives within `end_reached_threshold` of the last item — edge-triggered, re-armed on leaving, never for content that fits. The paging hook a virtualizer exists to serve: fetch the next page as they approach the end. Answered on iOS, web and macOS; a no-op on Android, Linux and Windows.",
                constraint: "inline prop or builder method `.on_end_reached(..)`; backends: ios, web, macos",
            },
            PropFieldSpec {
                name: "end_reached_threshold",
                type_str: "f32",
                doc: "How close to the end counts as arriving, in logical px. `0` (default) is the very end; a screenful prefetches the next page before the reader gets there.",
                constraint: "",
            },
            COMMON_STYLE_FIELD,
            COMMON_REF_FILL_FIELD,
