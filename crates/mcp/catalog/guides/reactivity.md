...
2. **The hoisted-snapshot trap** — pitfall 1 in disguise, and the easiest bug to ship:
   ...
   The component body runs once and is not a tracked context. Keep derivations behind `move ||` — `let too_short = memo(move || name.get().len() < 3);` gives a live `ReadSignal<bool>` condition — or inline the read into the condition (`if name.get().len() < 3`). Under the 0.4.0 inverted gate, any `ui!` `if`/`match` condition that *might read a signal* — a `.get()` or **any** call anywhere in the condition — lowers to a reactive branch; only a *provably signal-free* condition (a literal, a bare `bool` path, a call-free comparison) stays static. **A `let` freezes, a closure — or a call in the condition — flows.** The `snapshot-condition` lint flags the hoisted-`let` form (nothing warns at runtime — a component body is untracked by construction) — inline forms inside `ui!` (including a `for` header, next pitfall) are NOT linted; verify list reactivity by running the app. For an *intentional* build-time snapshot, declare it: `.peek()` reads without subscribing and silences the lint (on a `Reactive<T>` prop the same intent is spelled `.get_untracked()`; on a signal that name does not exist).

4. **Lists: iterate the signal, not `.get()`.** `for item in items.get()` inside `ui!` iterates a build-time snapshot — the loop renders once and NEVER re-runs, even though `items` is a signal. Unlike an `if`/`match` condition (reactive by default in 0.4.0), a `for` header's `.get()` does NOT make the loop reactive. The reactive form iterates the `Signal<Vec<T>>` itself, with a key for reconciliation:
