...
/// `framework_mcp::catalog_json()` to stdout and exit zero.
...
pub(crate) async fn preload_subprocess_catalog(
    ...
    match framework_mcp::ResolvedCatalog::build_from_json(&json) {
        Ok(c) => {
...
async fn run_subprocess_reload(
    ...
    let json = String::from_utf8_lossy(&output.stdout);
    let new_cat = match framework_mcp::ResolvedCatalog::build_from_json(&json) {
        Ok(c) => c,
        Err(e) => {
